...
### No. 3: 수영장 옆에 앉은 작은 체구의 스페인 여성 초상화
...
민트 그린 실내 수영장 옆에 앉아 있는 작은 체구의 스페인 여성의 세련된 세로형 미용 초상화입니다.
...
햇빛에 바랜 돌 아치형 출입구로 프레임된 두 여인의 이미지를 생성하기 위한 프롬프트입니다.
...
같은 여성을 다양한 각도에서 촬영한 네 장의 스튜디오 사진으로 구성된 캐릭터 시트를 생성하기 위한 프롬프트입니다.
...
참조 캐릭터를 기반으로 한 다양한 이모지 스티커를 3x3 그리드로 생성하는 프롬프트로, 밝은 그라데이션 배경에 중국어와 영어 텍스트가 포함됩니다.
...
### No. 7: 네 가지 도트 아트 캐릭터 동작 스프라이트 시트
...
네 가지 유형의 도트 아트 캐릭터 애니메이션을 각각 최대 4프레임까지 포함하고 일관된 디자인을 유지하는 스프라이트 시트를 생성하는 프롬프트입니다.
...
스케치를 바탕으로 핵심 주제, 감정, 제스처를 보존하면서 세부 사항을 재구성한 오리지널 아트워크를 제작하는 프롬프트입니다.
...
적갈색 머리를 가진 젊은 여성의 사실적인 아침 라이프스타일 초상화입니다.
...
현대식 홈 오피스의 소파에 편안하게 누워 있는 젊은 여성의 사실적인 스마트폰 사진을 생성하는 프롬프트입니다.
...
반사 배경이 있는 실내에서 젊은 여성의 사실적인 아이폰 초상화를 생성하는 프롬프트입니다.
...
투명 배경과 4x4 레이아웃으로 캐릭터 전투 동작을 위한 단순화된 128px 픽셀 아트 스프라이트 시트 생성을 위한 프롬프트입니다.
...
어린이 그림을 음침한 밤 캠프장으로, 그다음 낮 버전으로 변환하고, 마지막으로 캐릭터의 옷을 작업복으로 변경하는 일련의 프롬프트입니다.
...
하나의 연속된 선만을 사용하여 국가를 표현한 세로형 미니멀리스트 아트 프린트를 제작하기 위한 프롬프트입니다.
...
개인의 핵심 정체성과 특징을 보존하면서 1980년대의 모습으로 재해석하는 프롬프트입니다.
...
앞면과 뒷면이 보이는 두 개의 떠 있는 오버사이즈 티셔츠를 보여주는 3D 목업을 생성하기 위한 프롬프트입니다.
...
업로드된 사진을 얼굴 참고 자료로 사용하여 럭셔리 남성복 에디토리얼 콜라주를 제작하기 위한 프롬프트입니다.
...
업로드된 이미지를 사용하여 피사체의 얼굴 특징과 자연스러운 외모를 보존하도록 지시하는 프롬프트입니다.
...
업로드된 사진을 얼굴 정체성과 구도 참고 자료로 사용하여 전문적인 스튜디오 초상화를 제작하기 위한 프롬프트입니다.
...
얼굴 정체성 참고 자료를 사용하여 일관된 남성 모델이 등장하는 패션 콜라주를 제작하기 위한 프롬프트입니다.
...
미래형 우주선 조종석 안에서 자신감 넘치는 여성 우주비행사의 사실적인 초상화를 생성하기 위한 프롬프트입니다.
...
업로드한 이미지를 얼굴 참고 자료로 사용하여 풀바디, 초현실적인 스튜디오 패션 초상화를 생성하기 위한 프롬프트입니다.
...
프리미엄 스포츠 오토바이 옆에 앉아 있는 남성의 초현실적인 초상화입니다.
...
얼굴 정체성 참고 자료를 사용하여 남성의 초현실적인 에디토리얼 초상화를 생성하기 위한 AI 프롬프트입니다.
...
얼굴 정체성 참고 자료를 사용하여 스타일리시한 성인 남성의 프리미엄 패션 초상화를 생성하기 위한 프롬프트입니다.
...
참고 이미지를 기반으로 한 여성의 고품질, 초사실적인 캔디드 라이프스타일 초상화를 생성하기 위한 프롬프트입니다.
...
피사체 사진을 얼굴 참고 자료로 사용하여 현실적인 실외 겨울 패션 초상화를 생성하기 위한 AI 프롬프트입니다.
...
스타일리시한 성인 남성의 초현실적인 럭셔리 스튜디오 초상화를 생성하는 AI 프롬프트.
...
업로드한 이미지를 구도와 분위기 참고로 사용하여 시네마틱 빗물 초상화 콜라주를 생성하는 프롬프트.
...
특정 여성의 얼굴 특징과 외모를 충실히 보존한 3:4 비율의 초현실적인 스마트폰 셀피를 생성하는 프롬프트.
...
피사체의 정체성과 외모를 보존한 클로즈업 패션 초상화를 생성하는 프롬프트.
...
얼굴 참고 이미지를 사용하여 정장을 입은 젊은 남성의 클로즈업 스튜디오 초상화를 생성하는 프롬프트.
...
### No. 33: 정체성 보존이 적용된 시네마틱 라이프스타일 사진
...
특정 동아시아 여성의 정체성과 특징을 보존하며, 사실적인 시네마틱 이미지를 생성하기 위한 프롬프트.
...
### No. 34: 정체성 참조가 적용된 시네마틱 패션 에디토리얼
...
참조 이미지의 정체성을 보존하며, 어두운 배경의 여성에 대한 시네마틱 패션 사진을 생성하기 위한 프롬프트.
...
업로드된 이미지를 얼굴 정체성 참조로 사용하여 성인 남성의 초현실적 에디토리얼 초상화를 생성하기 위한 이미지 생성 프롬프트.
...
ChatGPT Astra를 사용하여 배신한 도끼 용사와 드래곤의 전투 장면을 생성하기 위한 프롬프트입니다.
...
역동적이고 사실적인 앵글에서 바라본 영웅적인 거대 로봇을 보여주는 이미지를 생성하기 위한 프롬프트입니다.
...
### No. 38: 업로드된 대상물을 기반으로 한 애니메이션 스타일
...
자신감 있는 선묘, 최소한의 셀 셰이딩, 밝고 채도 높은 색상을 특징으로 하는 애니메이션 스타일 이미지를 생성하기 위한 프롬프트입니다.
...
정면을 향한 플랫 레이아웃 스타일로 선명하고 가독성 좋은 라벨이 있는 두 가지 제품 이미지를 생성하기 위한 프롬프트입니다.
...
거대하고 위엄 있는 날개를 펼친 대머리독수리 앞에 중앙에 위치한 여성이 등장하는 시네마틱 판타지 장면을 위한 프롬프트입니다.
...
특정 모델의 얼굴 정체성을 보존하는 프리미엄, 초현실적인 하이패션 광고 이미지를 생성하기 위한 프롬프트입니다.
...
첨부된 기사를 바탕으로 일본 잡지 '애프터눈'의 스타일로 4페이지 분량의 만화를 생성하기 위한 프롬프트입니다.
...
엄격한 시각적 가이드라인을 사용하여 스플릿 스크린 방식의 캐릭터 레퍼런스 시트를 제작하기 위한 프롬프트입니다.
...
투명 배경의 단순화된 128px 픽셀 아트 전투 애니메이션 스프라이트 시트를 생성하기 위한 프롬프트입니다.
...
캐릭터의 전투 동작을 위한 16프레임 픽셀 아트 스프라이트 시트를 생성하기 위한 프롬프트입니다.
...
캐릭터의 전투 동작(대기, 베기, 방어, 넉다운 상태)을 위한 16프레임의 128px 단순화 픽셀 아트 스프라이트 시트를 생성하는 프롬프트입니다.
...
참조 사진을 사용하여 피사체의 신원을 보존한 전신의 사실적(포토리얼리스틱) 초상화를 생성하는 프롬프트입니다.
...
엄격한 시각적 가이드라인을 사용하여 두 캐릭터에 대한 스플릿 스크린 캐릭터 참조 시트를 생성하는 프롬프트입니다.
...
두 캐릭터에 대해 엄격한 시각적 참조를 사용하여 스플릿 스크린 캐릭터 시트를 생성하는 프롬프트입니다.
...
### No. 50: 돔 & 브라이언을 위한 스플릿 스크린 캐릭터 바이블
...
업로드된 참조 자료를 사용하여 두 캐릭터의 스플릿 스크린 캐릭터 시트를 생성하는 프롬프트입니다.
...
강렬한 타이포그래피와 엄격한 시각적 참조 준수를 특징으로 하는 캐릭터 '베이비'를 위한 프리미엄 캐릭터 시트 생성 프롬프트입니다.
...
강렬한 타이포그래피와 엄격한 시각적 참조를 갖춘 프리미엄 캐릭터 시트를 생성하는 프롬프트입니다.
...
엄격한 시각적 가이드라인을 사용하여 스플릿 스크린 캐릭터 참조 시트를 생성하는 프롬프트입니다.
...
엄격한 시각적 가이드라인을 사용하여 네오와 트리니티의 스플릿 스크린 캐릭터 참조 시트를 생성하는 프롬프트입니다.
...
엄격한 시각적 참조를 사용하여 '케빈 대 웻 밴디츠'를 위한 프리미엄 캐릭터 시트를 생성하는 프롬프트입니다.
...
시원한 시네마틱 느낌의 야간 옥상 계단 패션 에디토리얼을 위한 초현실적 사진 프롬프트입니다.
...
엄격한 시각적 가이드라인을 사용하여 두 캐릭터의 스플릿 스크린 캐릭터 참조 시트를 생성하는 프롬프트입니다.
...
### No. 59: 마이크와 마커스를 위한 프리미엄 시네마틱 캐릭터 바이블 시트
...
엄격한 시각적 가이드라인을 사용하여 스플릿 스크린 캐릭터 레퍼런스 시트를 생성하기 위한 프롬프트입니다.
...
### No. 60: 리와 카터를 위한 프리미엄 시네마틱 캐릭터 바이블 시트
...
엄격한 시각적 가이드를 사용하여 스플릿 스크린 캐릭터 레퍼런스 시트를 제작하기 위한 프롬프트입니다.
...
부드러운 이목구비와 흐트러진 검은 머리를 가진 여성이 뉴욕 지하철에서 자연스럽게 생활하는 모습의 캔디드 라이프스타일 사진을 위한 프롬프트입니다.
...
실제 여성 한 명을 정면, 3/4 측면, 프로필, 후면 뷰로 보여주는 캐릭터 레퍼런스 시트를 생성하기 위한 프롬프트입니다.
...
업로드된 사진을 사진 사실주의와 미니멀 수채화 비네트 스타일이 결합된 정교한 아트워크로 변환하기 위한 프롬프트입니다.
...
업로드된 각 사진을 병합하지 않고 별도의 고품질 포스터로 변환하는 JSON 지시사항입니다.
...
밤의 스터디 룸을 생성한 다음, 노트북 화면 내용만 자동화 플로우차트로 교체하기 위한 프롬프트입니다.
...
대화, 스토리, 캐릭터 설정을 보존하면서 이미지를 2페이지 스토리 만화 스타일로 다시 그리라는 프롬프트입니다.
...
특정 구도 요소를 보존하면서 환경 배경을 더 자유로운 회화적 스타일로 편집하라는 프롬프트입니다.
...
업로드된 이미지를 참고 자료로 사용하여 심하게 손상된 오래된 초상화를 깨끗하고 현대적으로 보이는 사진으로 복원하기 위한 프롬프트입니다.
...
원래의 형태, 질감, 재질을 보존하면서 물체의 색상을 네온 블랙으로 변경하는 프롬프트.
...
상단 패널을 원본 참조 사진으로 유지한 채, 두 개의 겹쳐진 패널로 구성된 단일 수직 이미지를 생성하기 위한 프롬프트.
...
주요 피사체를 자연스럽게 배치한 채 선택한 화면비로 사진 크기를 조정하기 위한 프롬프트.
...
특정 각도로 창문을 통해 들어오는 부드럽지 않은 강한 햇살에 의해 얼굴에 날카로운 그림자를 만들어내는 조명의 초상화를 묘사하는 프롬프트입니다.
...
주인공의 시점에서 바라본 가상의 차세대 3D 오픈월드 연애 RPG의 사실적인 스크린샷을 생성하기 위한 프롬프트입니다.
...
부드러운 파스텔 톤과 손으로 제작한 듯한 디테일로 유명한 구조물을 표현한 귀엽고 우아한 3D 디오라마를 생성하기 위한 프롬프트입니다.
...
잎사귀 드레스와 아이보리 우산을 든, 옆모습의 날씬한 여성을 묘사한 세로형 식물 종이 예술 작품입니다.
...
### No. 9: 흰색과 금색 팔라딘의 사진처럼 사실적인 모델 키트
...
전자제품 매장에 진열된 많은 균일한 박스들 사이에 전시된, 흰색과 금색 팔라딘 로봇의 사진처럼 사실적인 모델 키트 이미지를 생성하기 위한 프롬프트입니다.
...
밤에 빛나는 전통 중국식 건물과 역사적인 도시 성벽 옆에 서 있는 젊은 여성의 사실적인 캐주얼 여행 사진을 위한 프롬프트입니다.
...
1920-1930년대에서 영감을 받은, 계층화된 종이 릴리프와 엠보싱 일러스트레이션 스타일의 차분한 색상 팔레트를 가진 여행 포스터를 생성하기 위한 프롬프트입니다.
...
고양이 수염, 나비의 겹눈, 그리고 한 소녀의 피부를 포착한 사실적인 매크로 근접 촬영 사진을 위한 프롬프트입니다.
...
선명한 다이얼 텍스트가 있는 럭셔리 티타늄 크로노그래프 워치의 사실적인 잡지 광고 목업을 위한 프롬프트입니다.
...
산 라인 아트가 그려진 무광 블랙 커피 백을 러스틱한 나무 위에 배치한 사실적인 상업용 목업을 위한 프롬프트입니다.
...
가을 의상을 입은 젊은 여성 모델을 소프트한 파스텔 색상을 사용하여 담은 깔끔한 패션 룩북 포스터를 생성하기 위한 프롬프트입니다.
...
상징적인 랜드마크의 작은 3D 모델을 특징으로 하는 매력적인 수제 미니어처 장면을 생성하기 위한 프롬프트입니다.
...
황혼의 도시를 내려다보는 성인 동아시아 여성의 사실적인 에디토리얼 초상화를 위한 프롬프트입니다.
...
햇살이 드는 동네 카페에 있는 성인 동아시아 여성의 사실적인 시네마틱 초상화를 위한 프롬프트입니다.
...
피사체를 프레임 왼쪽 가장자리에 배치한 편집 스타일의 초상화를 제작하는 프롬프트입니다.
...
지정된 장소를 위한 고품질의 현대적인 아트북 스타일 포스터를 제작하는 프롬프트입니다.
...
밤에 평범한 아파트 환경에서 따뜻한 분위기의 여성을 담은 사실적인 16:9 가로 사진을 생성하는 프롬프트입니다.
...
일반적으로 싫어하는 음식들만으로 구성된 디럭스 파르페를 30가지 재료와 주석과 함께 생성하는 프롬프트입니다.
...
중세 일본 철도 관광 스타일의 프리미엄 세로형 여행 포스터를 제작하는 프롬프트입니다.
...
갈색 머리의 젊은 여성이 지나가는 열차 옆에 가만히 서 있는 고화질 초상화입니다.
...
정교한 메이크업과 여우 같은 얼굴을 가진 키 크고 날씬한 모델이 부드럽고 몽환적인 조명 아래 치파오를 입고 있는 고각도 초상화입니다.
...
황혼에 농가와 포플러 나무가 있는 홍수 평원을 묘사한 시네마틱 영화 스틸로, 물에 어울리지 않는 반사가 특징입니다.
...
16:9 화면비로 보컬로이드 MV 스타일의 다양한 샷을 3x3 그리드로 제작하는 프롬프트입니다.
...
16비트 스타일로 비행, 이륙, 착륙하는 까마귀의 스프라이트 시트를 생성하기 위한 프롬프트입니다.
...
바위산 길에서 바라본, 군마를 탄 검투사를 극단적인 저각에서 바라본 장면입니다.
...
암초를 확대하고, 돌에 비치는 빛을 관찰하며, 정확히 여덟 개의 팔을 포함하도록 지시하는 프롬프트입니다.
...
### No. 31: 모란 속 미니어처 중국 여성들의 시네마틱 애너모픽 스틸
...
커다란 연분홍색 모란 속에 앉아 있는 다섯 명의 미니어처 중국 여성들을 담은 와이드 애너모픽 사진입니다.
...
전경과 배경을 가득 메운 짙은 분홍색 꽃 가지에 둘러싸인 여성의 사진입니다.
...
일몰의 해변에 앉아 손에 쥔 모래를 바라보는 로봇의 리얼리즘 시네마틱 장면입니다.
...
네온 핑크와 그린 털 늑대 귀와 초커가 특징인 사이버펑크 스타일의 푹신한 의상을 생성하기 위한 프롬프트입니다.
...
주근깨와 바람에 날리는 머리를 가진 젊은 여성의 원본 아이폰 사진을 위한 프롬프트입니다.
...
### No. 36: 1980년대 스타일의 백악관 잔디밭에서 외계인을 맞이하는 대통령
...
백악관 잔디밭에서 외계인을 맞이하는 대통령의 1980년대 스타일 사진을 묘사한 이미지를 생성하기 위한 프롬프트입니다.
...
허구의 화려한 당나라 왕실 밤 연회를 묘사한 와이드스크린 역사 영화 스틸입니다.
...
검은색 긴 머리의 성인 동아시아 여성, 얼굴에 따뜻한 빛과 그림자, 자연스러운 피부 질감이 특징인 사실적 패션 사진 생성을 위한 프롬프트입니다.
...
### No. 39: 블랙 미스트 필터가 적용된 대만 여성 초상화
...
블랙 미스트 필터, 오버레이 마스크, 은은한 하이라이트가 있는 소프트라이트 CCD 사진 기법의 초상화 생성을 위한 프롬프트입니다.
...
비가 그친 후 강가에 위치한, 빗방울이 맺힌 단일 붉은 상사화의 사실적 이미지 생성을 위한 프롬프트입니다.
...
젖은 양모, 피부, 가로등에 초점을 맞춘, 영업 종료 후의 한 사람을 묘사하는 분위기 있는 이미지 생성을 위한 프롬프트입니다.
...
바람으로 스타일링된 세 명의 인물, 강렬한 햇빛, 풍부한 실크가 특징인 패션 에디토리얼 장면 생성을 위한 프롬프트입니다.
...
주차장에 주차된 테슬라 모델 Y를 0.5배 카메라로 찍은 사실적 아이폰 사진 생성을 위한 프롬프트입니다.
...
황혼의 생태 미래 도시, 거대한 지구가 보이는 달 도시, 은하의 수호 기사 세라핌, 얼음 수정 성당에서 춤추는 은발 여왕을 묘사하는 프롬프트입니다.
...
100명 이상의 군대, 캐릭터 루나, 흰색 용, 일본어 간판, 정교한 창 다루기, 비에 반사된 빛이 특징인 복잡한 장면 생성을 위한 프롬프트입니다.
...
토끼가 뛰어오르는 이미지와 회색 네덜란드 드워프 토끼가 뛰어오르는 이미지 생성을 위한 프롬프트입니다.
...
큰 공원에서 캐주얼 복장을 한 애니메이션 스타일의 젊은 남녀를 초고해상도 16K로 렌더링하고 대비와 채도를 강화한 이미지 생성 프롬프트입니다.
...
홍루몽의 한 장면을 전통 중국식 장권화로 재구성하라는 프롬프트입니다.
...
사막 브루탈리즘 스타일의 거대한 토속 건축물을 유명 건축가들을 혼합한 형태로 표현한 이미지 생성 프롬프트입니다.
...
골든 아워에 나무가 늘어선 거리를 걷는 여성을 스마트폰으로 찍은 것처럼 사실적인 사진을 위한 프롬프트입니다.
...
산토리니, 아말피 해안, 파리를 모티브로 한 고급스러운 럭셔리 여행 포스터 컬렉션을 제작하기 위한 프롬프트입니다.
...
햇살 좋은 오후, 평화로운 도시 공원에서 긴 갈색 머리의 젊은 여성을 찍은 캔디드 야외 인물 사진 프롬프트입니다.
...
치명적인 사막 암살자를 주인공으로 한 시네마틱 캐릭터 레퍼런스 시트를 생성하기 위한 프롬프트입니다.
...
밤의 프라이빗 라운지에 있는 아름다운 동아시아 여성의 초현실적인 스마트폰 패션 사진을 위한 프롬프트입니다.
...
한 나라의 상징적인 랜드마크와 문화적 요소를 담은 빈티지 스타일 우표를 제작하기 위한 프롬프트입니다.
...
알아볼 수 있는 건축적 랜드마크를 주제로 한 스타일화된 수제 빈티지 여행 포스터를 제작하기 위한 프롬프트입니다.
...
현대식 지하 주차장에 있는 아름다운 유라시안 여성의 초현실적인 인물 사진 프롬프트입니다.
...
세라믹 접시 위에 놓인 특정 음식 요리를 중심으로 한 매력적인 미니어처 장면을 생성하기 위한 프롬프트입니다.
...
알려진 상점이나 브랜드를 3D 클레이 미니어처 디오라마로 변환하여 기발한 느낌을 생성하기 위한 프롬프트입니다.
...
중앙에 위치한 상징적인 구조물을 중심으로 한 디테일하고 아늑한 미니어처 세계를 창조하기 위한 프롬프트입니다.
...
흰색 스튜디오 배경을 앞에 두고 비대칭 구도로 촬영된 남성의 클로즈업 초상화를 위한 프롬프트입니다.
...
시간이 지나면서 풍화 작용으로 형성된 하얀색 미로의 풍경을 생성하기 위한 설명적 프롬프트입니다.
...
밤에 적이 아닌 단 하나의 작은 돌에 의해 멈춰 선 화이트 드래곤 기사를 묘사한 장면으로, 철학적인 캡션이 함께합니다.
...
짧은 검은 머리의 자신감 넘치는 남성의 프리미엄 에디토리얼 스타일 스튜디오 초상화를 생성하기 위한 프롬프트입니다.
...
따뜻하고 무한 배경의 스튜디오 환경에서 자신감 있는 남성 모델의 풀바디 초상화를 생성하기 위한 프롬프트입니다.
...
알프스, 두바이, 유럽 샬레 경험을 결합한 정교한 럭셔리 라이프스타일 여행 캠페인을 위한 프롬프트입니다.
...
빗, 머리끈, 신발끈이 그들의 일을 마무리하는 동안 보고서를 읽는 푸른 마법사 대가의 아침 풍경을 묘사한 장면입니다.
...
하얀 흙, 소금, 석회암으로 이루어진 해안에 배가 도착하고, 그 너머에는 보이지 않는 땅이 펼쳐진 장면을 묘사하는 프롬프트입니다.
...
바다 근처에 살며 바다의 징후를 읽는 해안 어부인 엘프를 묘사하는 프롬프트입니다.
...
성인 남성의 전문적인 스튜디오 헤드샷 초상화를 생성하기 위한 프롬프트입니다.
...
### No. 71: 럭셔리한 모던 인테리어 속 자신감 있는 남성
...
어두운 목재 출입구와 큰 거울이 있는 럭셔리한 모던 인테리어 속 자신감 있는 남성의 초현실적인 라이프스타일 초상화입니다.
...
이세계 지도 앞, 책상 나뭇결을 기억하는 창병의 밤을 묘사한 시적인 프롬프트.
...
드래곤 날개 아래에서 휴식을 취하는 두 명의 드래곤 기사와 지도, 딱딱한 빵이 있는 장면.
...
초저녁 시간대의 고급 도서관 라운지에서 휴식을 취하는 남성의 초현실적인 초상화.
...
밤하늘 아래 붉은 상사화가 늘어선 가을 길을 걷는 밤 우편 배달부.
...
미니멀리스트 스튜디오 설정에 앉아 깔끔한 정장을 입은 당당한 흑인 남성의 초현실적인 에디토리얼 초상화.
...
햇빛과 자갈길이 그녀에게 닿기 전에 마법으로 사라지는 푸른 마법사 대장의 오후 장면.
...
지표 균열에서 얇은 불꽃이 치솟고 먼 곳에 카스피해가 보이는 황혼의 황야 장면을 묘사하는 프롬프트.
...
극적으로 펼쳐진 바다 노을 풍경 속 빈티지 목재 범선의 초현실적인 이미지.
...
달빛이 비치는 용 마구간에서 백룡 기사장이 잠든 용의 왼쪽 날개 부상을 조심스럽게 확인하는 장면.
...
### No. 81: 화산 도시에서 푸른 여행 천을 수선하는 여행자
...
오후, 먼 산을 배경으로 화산 도시에서 푸른 여행 천을 수선하는 장면.
...
최고의 칭호를 잃은 창을 든 병사가 거울 세계의 아침, 젖은 신발끈부터 하루를 시작하는 장면을 묘사한 프롬프트.
...
그녀의 몸이 필요하지 않았던 시절, 책, 의자, 차와 함께 새벽 전의 청마법사장 루나를 묘사한 장면.
...
건조한 황야에서 불이 땅에서 샘솟고, 회색 진흙이 흐르며 먼 곳에 카스피해가 보이는 풍경에 대한 묘사적 프롬프트.
...
엘 드라고가 옆에서 잠든 채, 창의 굳은살을 확인하는 밤의 흑룡 기사 루나를 묘사한 장면.
...
페른과 프리렌 캐릭터 간의 대화를 묘사한 안전한 작업 환경(SFW) 일러스트레이션.
...
녹색이 사라지며 오크르, 녹슨 색, 회백색, 연두색의 층이 드러난, 바람과 비에 깎인 언덕 풍경을 시적으로 묘사한 프롬프트.
...
흰 벽과 대나무 그림자를 배경으로 원형 부채를 든 고전적인 여인이 등장하는 동양 미학의 배경화면을 생성하는 AI 프롬프트입니다.
...
도쿄 경찰 중장비 로봇 팀이 구명정을 끌며 피난민을 구조하는 장면을 묘사하는 프롬프트입니다.
...
천문 관측대에서 도시 탐사를 즐기는 모습을 담은 밝은 주간의 CCD 스타일 라이프스타일 사진을 생성하는 프롬프트입니다.
...
젊은 여성의 리얼하고 자연스러운 아이폰 스타일 인물 사진을 생성하는 프롬프트입니다.
...
비취색 꽃창가에 서 있는 고전적인 여인이 등장하는 우아하고 미니멀한 동양풍 포스터를 생성하는 이미지 생성 프롬프트입니다.
...
### No. 93: 동아시아 여성의 사진처럼 사실적인 편집용 인물 사진
...
특정 얼굴 특징을 가진 젊은 동아시아 여성의 무릎 위, 눈높이에서 찍은 사진처럼 사실적인 인물 사진을 생성하는 상세 프롬프트입니다.
...
### No. 94: 강을 막는 쓰레기를 치우는 경찰 중장비대 로봇
...
강을 막고 있는 쓰레기를 치우는 경찰 중장비대 소속 로봇 분대를 묘사하는 프롬프트입니다.
...
### No. 95: 하트형 얼굴의 사진처럼 사실적인 편집용 인물 사진
...
특정 얼굴 특징을 가진 젊은 동아시아 여성의 무릎 위, 눈높이에서 찍은 편집용 인물 사진을 생성하는 상세 프롬프트입니다.
...
아이폰 메인 카메라로 찍은 것처럼 20~26세 여성의 리얼하고 캐주얼한 9:16 세로 비율 사진을 생성하는 프롬프트입니다.
...
미니멀하고 상쾌한 스타일로 물가에 서 있는 여인이 등장하는 동양 미학의 배경화면을 생성하는 프롬프트입니다.
...
특정 얼굴 특징을 가진 젊은 동아시아 여성의 사진처럼 사실적인 편집용 인물 사진(허리 위 구도)을 생성하는 상세 프롬프트입니다.
...
도시적이고 예술적인 배경과 특정 의상을 갖춘 밝은 주간 CCD 스타일 라이프스타일 사진을 생성하는 프롬프트입니다.
...
### No. 100: 동아시아 여성의 사진처럼 사실적인 편집용 인물 사진
...
특정 얼굴 특징을 가진 젊은 동아시아 여성의 무릎 위, 허리 높이에서 찍은 편집용 인물 사진을 생성하는 상세 프롬프트입니다.
...
벚꽃이 흩날리는 파란 문을 지나가는 우아한 고풍 스타일의 여인이 등장하는 동양 미니멀리즘 배경화면을 생성하는 프롬프트입니다.
...
호우로 침수된 도로에서 차량을 옮기고 있는 중기대처반 소속 경찰 로봇 팀의 이미지입니다.
...
### No. 103: 낮 시간 밝은 CCD 도시 스포츠 라이프 사진
...
도시 스포츠 환경에서 낮 시간에 촬영한 밝은 CCD 스타일의 라이프스타일 사진을 생성하기 위한 프롬프트입니다.
...
현대적 동양 여성 미학과 아침의 분위기가 담긴 밝고 깔끔한 9:16 비율의 미니멀리스트 세로형 커버를 제작하기 위한 프롬프트입니다.
...
둥근 창가에서 휴식을 취하는 여성이 등장하는 동양 선(禪) 미니멀리스트 스타일의 포스터를 생성하기 위한 프롬프트입니다.
...
폭주하는 트럭을 멈추려고 시도하는 중기대처반 소속 경찰 로봇 분대의 이미지입니다.
...
고리 귀걸이를 착용한 젊은 일본 또는 한국 여성의 전신, 아이레벨 에디토리얼 포트레이트를 위한 프롬프트입니다.
...
굵은 흰색 테 두꺼운 안경을 쓴 젊은 일본 또는 한국 여성의 무릎 높이에서 바라본 반신 포트레이트입니다.
...
### No. 109: 여성과 풍경이 등장하는 선(禪) 미니멀리스트 커버 포스터
...
창가에서 풍경을 만지고 있는 여성을 미니멀리스트 동양 선(禪) 스타일로 표현한 이미지를 생성하기 위한 프롬프트입니다.
...
### No. 110: 푸른 식물로 뒤덮인 포스트 아포칼립스 도시
...
푸른 식물이 무성하게 자라난 중국 선전의 포스트 아포칼립스 도시를 디테일하게 담은 사진입니다.
...
고요한 밤 풍경 속에서, 너구리 같은 얼굴을 가진 가상의 젊은 일본 여성의 사실적인 이미지를 생성하기 위한 프롬프트입니다.
...
여성 전용 아쿠아리움을 방문한 여성들을 360도 유리 탱크 너머로 바라보는 돌고래의 시점에서 본 사실적인 이미지입니다.
...
아늑한 보도 디테일이 있는 브랜드 매장의 미니어처 클레이 모델을 생성하기 위한 프롬프트입니다.
...
코발트 블루 하늘을 배경으로 화려한 의상을 입은 여섯 명의 사람들이 등장하는 가상 브랜드의 패션 광고 캠페인으로, 낮은 앵글에서 촬영되었습니다.
...
### No. 115: 벚꽃 비 내리는 손으로 그린 애니메이션 결투 장면
...
1990년대 후반 일본 손으로 그린 애니메이션 영화의 한 장면으로, 벚꽃이 흩날리는 아래에서 카타나를 든 여성이 검사와 대치하고 있습니다.
...
어두운 로맨틱한 우아함을 지닌 고딕 할로윈 호러 신부 코스튬을 위한 프롬프트입니다.
...
### No. 117: 동아시아 여성의 무드 있는 시네마틱 초상화
...
창백한 피부와 긴 검은 머리를 가진 젊은 동아시아 여성의 고대비 시네마틱 클로즈업 초상화를 위한 프롬프트입니다.
...
### No. 118: 젖은 횡단보도 위의 시네마틱 야간 거리 초상화
...
야간 젖은 횡단보도 위에 서 있는 젊은 동아시아 여성의 시네마틱 초상화를 위한 프롬프트입니다.
...
젯블랙 머리카락과 빛나는 황금색 눈을 가진 젊은 동아시아 여성의 시네마틱 클로즈업 초상화를 위한 프롬프트입니다.
...
안경을 쓰고 웨이브 머리를 한 젊은 동아시아 여성이 살며시 윙크하는 포토리얼리즘 초상화입니다.
...
### No. 121: 뒤엉킨 머리의 아날로그 필름 스타일 초상화
...
지저분한 검은 머리를 한 젊은 동아시아 여성이 카메라를 바라보는 아날로그 필름 스타일의 포토리얼리즘 초상화입니다.
...
고요한 물속에 젖은 머리로 떠 있는 평온한 젊은 여성의 시네마틱 초상화입니다.
...
빛나는 피부, 긴 검은 머리, 감은 눈을 가진 젊은 동아시아 여성의 포토리얼리즘 스튜디오 뷰티 초상화입니다.
...
연한 베이지색 히잡을 쓰고 차 안에 있는 젊은 동아시아 여성의 포토리얼리즘 클로즈업 초상화입니다.
...
나무 테이블에서 연한 색상의 히잡을 쓴 젊은 여성이 셀카를 찍는 이미지를 생성하기 위한 프롬프트입니다.
...
어깨 너머를 바라보는 젊은 동아시아 여성의 포토리얼리즘 초상화를 생성하기 위한 상세한 프롬프트입니다.
...
안경을 쓰고 웨이브 머리를 한, 평온한 표정의 젊은 동아시아 여성 초상화입니다.
...
도자기 같은 피부, 아몬드 눈, 선명한 립스틱을 가진 젊은 동아시아 여성의 포토리얼리즘 초상화입니다.
...
이슬 맺힌 피부와 젖은 검은 머리를 가진 젊은 여성이 어깨 너머를 바라보는 포토리얼리즘 초상화입니다.
...
검은 배경을 배경으로 긴 어두운 웨이브 머리를 가진 젊은 동아시아 여성의 클로즈업 초상화입니다.
...
긴 웨이브 머리, 둥근 안경, 부드러운 표정을 가진 젊은 동아시아 여성의 포토리얼리즘 초상화입니다.
...
웨이브진 밝은 갈색 머리와 평온한 표정을 가진 젊은 동아시아 여성의 시네마틱 클로즈업 초상화를 위한 프롬프트입니다.
...
짧은 검은 단발, 스모키 아이섀도우, 빛나는 피부를 가진 젊은 동아시아 여성의 포토리얼리즘 클로즈업 초상화입니다.
...
흰색 히잡과 검은색 야구 모자를 쓰고 차분한 표정을 지은 피부가 밝은 젊은 여성의 클로즈업 초상화.
...
먼지빛 토우프 색상의 시폰 히잡을 쓴, 섬세한 이목구비와 밝은 피부를 가진 젊은 동아시아 여성의 클로즈업 셀피 초상화.
...
### No. 136: 눈물 흘리는 동아시아 여성의 영화적 초상화
...
윤기 나는 눈물 자국이 있는 피부와 가늘게 흐르는 앞머리를 가진 젊은 여성의 사실적인 클로즈업 초상화.
...
검은색 단발머리에 한쪽은 붉은 눈이 보이고 다른 쪽은 보석이 달린 검은색 레이스 안대로 가린 창백한 피부의 젊은 여성 클로즈업 초상화.
...
길고 검은 머리를 가진 젊은 여성이 카메라를 정면으로 바라보는 클로즈업 영화적 초상화.
...
### No. 139: 윤기 나는 검은 머리를 한 클로즈업 초상화
...
밝은 피부와 긴 검은 머리를 가진 젊은 여성의 초상화를 생성하기 위한 상세한 프롬프트.
...
### No. 140: 햇살에 그을리고 땀 반짝임이 있는 미소 짓는 초상화
...
헝클어진 검은 머리에 눈을 가늘게 뜨고 미소 지으며 홍조가 돌고 촉촉한 피부를 가진 젊은 동아시아 여성의 클로즈업 초상화.
...
### No. 141: 윙드 아이라이너가 있는 표현력 풍부한 초상화
...
특정한 얼굴 특징과 메이크업을 가진 젊은 동남아시아 여성의 클로즈업 초상화를 생성하기 위한 상세한 프롬프트.
...
눈을 감고 편안한 표정을 지은 젊은 동아시아 여성의 클로즈업 셀피 초상화를 위한 프롬프트.
...
밝은 피부톤, 부드러운 메이크업, 온화하고 친근한 표정을 가진 젊은 동아시아 또는 인도네시아 여성의 초상화를 생성하기 위한 프롬프트.
...
특정한 외모 특징과 스타일링을 가진 젊은 동아시아 여성의 사실적인 셀피 초상화를 생성하기 위한 프롬프트.
...
인간 관찰자의 시점에서 블랙홀에 삼켜지는 지구의 시각적 단계를 묘사한 네 장의 그림을 생성하기 위한 프롬프트.
...
사이버펑크 카타나 전사와 거대한 기계 보스 개체, 두 캐릭터를 묘사하는 프롬프트입니다.
...
장엄한 중국 천상의 여신이 의례용 한복을 입고 장미빛 황금 구름 사이로 천상의 궁전 앞에 강림하는 이미지입니다.
...
아이보리색 한복을 입은 선녀가 아침 햇살에 비추어지고, 흐르는 리본과 금빛 비녀를 장식한 이미지입니다.
...
비단 한복을 입고 고요한 중국 선녀가 청록색 연못가에서 연꽃을 만지는 이미지입니다.
...
자수 장식 한복을 입고 흐르는 비단 리본을 두른 우아한 중국 천상의 여신이 흰 옥 계단 위에 서 있는 이미지입니다.
...
노(儺) 가면과 촛불이 있는 어두운 고대 중국 제사 사당 안에 있는 성인 동아시아 여성의 사실적인 영화적 초상화입니다.
...
젖은 검은 머리와 스모키 메이크업을 한 동아시아 여성이 어두운 고대 사당에 있는 사실적인 영화적 초상화입니다.
...
### No. 153: 밤의 의례 축제 동안 낡은 노(儺) 가면 옆에 서 있는 성인 동아시아 여성의 근접 초상화입니다.
...
날카로운 테두리의 유리잔에 짙고 폭포처럼 쏟아지는 거품 크라운이 있는 맥주의 사실적인 이미지를 생성하기 위한 프롬프트입니다.
...
다공성 두부 큐브가 냄비 위로 들어 올려지고, 단 하나의 호박색 국물 방울이 매달려 있는 이미지를 묘사하는 프롬프트입니다.
...
뚜렷한 빨간 고추 큐브, 차색으로 변한 채소, 밝은 녹색 피망, 연한 색 콩, 투명한 호박색-빨간 기름이 있는 한 숟가락의 고추 릴리시 이미지입니다.
...
아늑한 모던 거실에서 장난기 가득한 표정을 지은 젊은 여성의 초현실적으로 사실적인 클로즈업 초상화입니다.
...
밤에 어둡게 조명된 침실에서 침대 위에 반쯤 누워 있는 젊은 여성의 사실적인 수직 초상화를 위한 프롬프트입니다.
...
따뜻한 주광에 부드럽게 비춰지는 키 큰 풀밭에 누워 있는 젊은 동아시아 여성의 영화에서 영감을 받은 이미지입니다.
...
햇빛이 비치고 바람이 부는 날, 밝은 푸른 항구 옆에 무심코 쪼그리고 앉아 있는 젊은 동아시아 여성의 필름 그레인 스타일 초상화입니다.
...
파리 풍의 쇼핑 거리에 있는 젊은 아시아계 여성의 고해상도 자연광 스트리트 초상화를 위한 프롬프트입니다.
...
밤에 기차 창가에 앉아 있는 성인 동아시아 여성의 사실적인 캔디드 초상화입니다.
...
시계 뒤에 원형의 얼음과 연기 충격파가 있으며 깔끔한 실루엣을 유지하는 이미지입니다.
...
깊이감을 창출하기 위해 서로 다른 높이의 층으로 배치된 프리미엄 베이커리 진열 이미지 생성을 위한 프롬프트입니다.
...
### No. 165: 슬립 드레스를 입고 부드럽게 조명된 침실 초상화
...
부드럽게 조명된 침실에서 아이보리 색상의 새틴 슬립 드레스를 입은 성인 동아시아 여성의 사실적인 초상화입니다.
...
강력한 자동차가 뒤로는 거대한 먼지 기둥을 일으키면서도 전경에서는 깨끗하고 조용한 모습을 보여주는 이미지입니다.
...
### No. 167: 천체 불꽃에 배경 조명을 받는 마녀 실루엣
...
거대한 주황색-황금빛 천체 불꽃 구름을 배경으로 한 마녀의 실루엣 이미지입니다.
...
### No. 168: 코샤리 그릇 위로 떠오르는 토마토 소스 하트
...
윤기가 흐르는 토마토 소스가 코샤리 그릇 위에서 하트 모양을 이루는 이미지입니다.
...
손으로 만든 면발로 조각되어 실루엣이 알아볼 수 있는 체스 말을 생성하기 위한 프롬프트입니다.
...
### No. 170: 복싱 링에서 승리한 황금 복서 팔이 달린 참기름 병
...
거대한 참기름 병에 황금빛 복서 팔이 달려 복싱 링에서 승리한 모습을 보여주는 이미지입니다.
...
아침 식사 카운터, 연속된 조리대, 스테인리스 싱크대, 스토브탑, 프라이팬이 있는 소형 모던 아파트 주방 이미지를 생성하기 위한 프롬프트입니다.
...
로켓처럼 각도를 준 샤워마 랩 이미지로, 열린 속재료를 초점으로 강조합니다.
...
달리는 바나나 마스코트가 거대한 차가운 알루미늄 캔을 들고 카메라를 향해 내밀고 있는 이미지입니다.
...
특정한 특징과 조명을 가진 동아시아 여성의 사실적인 캔디드 사진을 생성하기 위한 프롬프트입니다.
...
바닐라 크림 층이 올라와 매끄러운 미니어처 스케이트장을 형성하며 규모를 겨울 세계로 변모시키는 쿠키 이미지입니다.
...
완벽하게 정렬된 겹겹이 쌓인 과자로 구성된 롤러코스터 트랙 이미지로, 완전한 루프를 특징으로 합니다.
...
### No. 177: 주요 버킷 뒤의 세 개의 깔끔한 레드 스포트라이트 구역
...
주요 버킷 뒤에 있는 세 개의 깔끔한 레드 스포트라이트 구역과 그 안의 보조 라이프스타일 순간들을 묘사하는 이미지 프롬프트입니다.
...
단일한 윤기나는 면발 흐름을 중심 수직 요소로 한 미니멀리스트 포스터 컨셉입니다.
...
보이는 붓질 질감과 평면적인 색상 블록이 특징인 회화적이고 구아슈 컨셉 아트 스타일의 시네마틱 2.5D 애니메이션을 만들기 위한 프롬프트입니다.
...
지저분한 머리의 남자가 그림자에서 모습을 드러내는 시네마틱 초상화로, 극적인 조명과 초현실적인 질감이 특징입니다.
...
거인으로 묘사된 잘생긴 젊은 남자가 자유의 여신상 옆에 서 있는, 초현실적인 시네마틱 여행 사진입니다.
...
골든아워 시간대에 고풍스러운 현관에서 차를 즐기고 있는 젊은 남자의 따뜻하고 자연스러운 라이프스타일 초상화입니다.
...
사이프러스와 소나무가 있는 푸른 초원에 서 있는 카우보이 모자를 쓴 여성의 초현실적인 초상화입니다.
...
특정한 특징을 가진 젊은 동아시아 여성의 수직 캔디드 사진을 생성하기 위한 프롬프트입니다.
...
찢겨진 포장지로 입을 벌린 듯한 효과를 내며 디저트 테마의 세계 안에 있는 대각선으로 포장된 디저트 바 이미지를 생성하기 위한 프롬프트입니다.
...
수직 타원형 거울에 비친 여성의 뷰티 초상화로, 매트 코발트 립스틱이 특징입니다.
...
사막에서 솟아오르는 거대한 맥주잔이 모래와 맥주로 뒤틀린 소용돌이를 이루는 초현실적인 이미지입니다.
...
오른쪽 하단 전경에 크고 선명하며 부분적으로 잘린 실제 맥주병이 등장하는 이미지를 위한 프롬프트입니다.
...
몬드리안의 '빨강, 파랑, 노랑의 구성'을 색상이 있는 서랍이 달린 기능성 데스크탑 수납장으로 변환하기 위한 프롬프트입니다.
...
상상 속 초현실적인 주제를 매우 사실적으로 생성하기 위한 프롬프트입니다.
...
귤 속에서 사는 아주 작은 인간들의 이미지로, 과일의 쪽이 건축물을 형성합니다.
...
까만 까마귀 같은 머리와 핏빛 붉은 입술을 가진 위협적인 인물을 날카로운 측면에서 바라본 이미지를 생성하기 위한 프롬프트입니다.
...
젊은 칼렌진 모델들을 내세운 데이팅 앱 광고를 생성하기 위한 프롬프트입니다.
...
해뜨기 전, 라디오를 든 풍파에 지은 얼굴의 인물이 옥상에 있는 인물 사진을 생성하기 위한 프롬프트입니다.
...
교토 후시미 이나리 신사의 상징적인 주홍색 토리이(문)가 있는 고요한 이른 아침 풍경을 여행 캠페인 이미지 스타일로 생성하기 위한 프롬프트입니다.
...
사용자의 과거 AI 예술 선호도를 바탕으로 그들의 내면 세계를 반영한 일러스트레이션을 요청하는 프롬프트입니다.
...
빛나는 도시를 담고 있는 방 크기의 투명 프로세서의 접사(매크로) 샷을 위한 프롬프트입니다.
...
언어, 코드, 창의성이 빛으로 연결되어 있는 상징적 자화상을 묘사한 이미지입니다.
...
### No. 199: 지중해풍 레트로 퓨처리즘을 가진 외계 세계
...
레트로 퓨처리즘 요소가 있는 지중해풍 건축물을 특징으로 하는 외계 세계의 인간 정착지를 생성하는 프롬프트입니다.
...
다양한 왕실적이고 슬픈 칭호로 묘사된 얼음의 군주이자 주권자인 차리차의 초상화를 생성하는 프롬프트입니다.
...
아침 안개 속 버스 정류장의 시적인 이미지로, 아무도 오기 전에 증기가 피어오르고 가장 이른 열차는 마음속의 생각입니다.
...
밤의 세탁소를 묘사한 장면으로, 밖에는 해파리가 있고, 한 소녀가 우주 일기예보를 확인하고 있습니다.
...
레이어드 갑옷과 로브를 입고 긴 검을 휘두르는 키 크고 우아한 동아시아 여성 전사의 9:16 비율 초상화를 생성하는 프롬프트입니다.
...
레트로 폰 미학으로 촬영된, 시골 배경의 젊은 동아시아 여성을 묘사한 9:16 비율 이미지 프롬프트입니다.
...
고요한 전통 정원에 앉아 있는, 새로운 중식 스타일 치파오를 입은 두 젊은 동아시아 여성의 초현실적 사진을 생성하는 프롬프트입니다.
...
시원하고 몽환적이며 고급스러운 회색 라이프스타일 스타일의 젊은 동아시아 무용수의 리얼리즘적인 클로즈업 셀카를 위한 9:16 세로 비율 프롬프트입니다.
...
콘서트에서의 20세기 K-pop 여성 아이돌을 담은 시네마틱하고 에디토리얼 스타일의 사진 프롬프트입니다.
...
### No. 208: 석조 발코니에 기대 선 우아한 동아시아 여성
...
고전적인 석조 난간에 기대어 서 있는 아름다운 성인 동아시아 여성의 9:16 세로형 패션 사진을 위한 프롬프트입니다.
...
빛나는 하늘과 높이 솟은 구름에 초점을 맞춘 디테일하고 낭만적인 미술 풍경화를 생성하기 위한 프롬프트입니다.
...
카카리코 마을을 통해 푹신한 퍼펫 닭들의 성난 무리에게 쫓기는 링크의 이미지입니다.
...
안개 낀 고요한 아침의 발리 쌀 테라스 장면을, 여행 캠페인 스타일로 표현한 이미지 프롬프트입니다.
...
코발트 디더 모뉴멘탈리즘 스타일로, 플랫폼을 가로지르며 점프하는 오리지널 페르시아의 왕자 캐릭터를 묘사한 이미지를 생성하기 위한 프롬프트입니다.
...
삶과 죽음의 경계에 서서 차가운 밤 안개 속에서 조용한 작별을 위한 인도자의 손을 내미는, 죽음의 사신인 소녀를 시적으로 묘사한 프롬프트입니다.
...
밤이 검은색으로 스며들어 오직 금빛만이 부적의 존재를 비추는 장면으로, '밤산책'과 '악귀퇴치'라는 단어가 등장합니다.
...
삶과 죽음의 경계에 선 비인간 소녀가 신부가 되는 꿈을 꾸며 자신의 모습을 비추어 보는 것을 시적으로 묘사한 프롬프트입니다.
...
비 오는 날, 오래된 방에서 책을 읽고 있는 두려움의 존재 '후후'와 그곳에 들어온 작은 새의 장면입니다.
...
작은 동물들이 제복을 입고 편지를 분류하는 우체국으로 변형된 부드러운 구름의 기발한 장면으로, 부드럽고 힐링되는 그림책 스타일로 렌더링되었습니다.
...
커스텀 오토바이를 탄 스타일리시한 수염 난 남성의 초현실적 시네마틱 초상화입니다.
...
곱슬머리에 레이어드 의상을 입은 젊은 남성의 흑백 하이패션 초상화를 만들기 위한 프롬프트입니다.
...
크림색 종이 위에 우아한 금색과 네이비 선으로 작업된 정사각형 형식의 아르데코 포스터 일러스트레이션입니다.
...
어두운 배경 위에 금색 윤곽선과 광택 표면을 가진 프리미엄 에나멜 핀 8개 컬렉션을 생성하기 위한 프롬프트입니다.
...
특정 레이블과 UI 요소를 갖춘 다크 테마의 SaaS 분석 대시보드 인터페이스를 생성하기 위한 프롬프트입니다.
...
크리에이티브 디자인 회사의 중국어 웹사이트를 위한 정면, 원근감 없는 데스크톱 웹페이지 전체 스크린샷을 생성하기 위한 프롬프트입니다.
...
송나라 시대 규방에서 송옥의 완전한 의상 착용 과정을 묘사한 전문적이고 고급스러운 고대 중국 패션 사진 시리즈를 엄격한 3x3 그리드로 배열하여 생성하기 위한 프롬프트입니다.
...
15:32를 가리키는 아날로그 시계, 구운 황금빛 프로볼로네 치즈가 든 햄버거, 그리고 유리잔이 있는 이미지를 생성하기 위한 프롬프트입니다.
...
파란색과 흰색을 사용한 정사각형 SNS 이미지로, 스마트폰 하나, 이미지 카드 세 장, 일본어 제목, 넓은 여백을 포함하며 추가 텍스트는 없습니다.
...
스페인어 물리학 책을 위한 전자기학 챕터를 생성하기 위한 프롬프트입니다.
...
우주 개발에 관한 세로형 흑백 1페이지 만화를 생성하기 위한 프롬프트입니다.
...
왼쪽 상단 뒤에 빨간 공, 오른쪽 하단 앞에 파란 공이 들어 있고, 큐브 위에는 녹색 공이 있으며, 노란색 기둥이 왼쪽 하단에서 오른쪽 상단을 관통하는 투명 유리 큐브의 아이소메트릭 투시도 생성을 위한 프롬프트입니다.
...
모델 리스트가 포함된 타이포그래피 포스터와 마스트헤드 및 커버라인이 있는 매거진 커버 생성을 위한 프롬프트입니다.
...
어두운 에메랄드 스튜디오에서 큰 시계를 강조하는 세로형 럭셔리 광고 프롬프트입니다.
...
구성선과 특정 레이아웃을 포함한 회화적 캐릭터 디자인 레퍼런스 시트를 생성하기 위한 프롬프트입니다.
...
두 오리지널 캐릭터를 위한 프리미엄 분할 화면 캐릭터 시트를 생성하기 위한 프롬프트입니다.
...
남성을 세 겹으로 구성한 초현실적인 기업 초상화 콜라주.
...
원본 사진과 동일한 장면의 손그림 일러스트레이션을 결합한 세로 이미지를 생성하는 프롬프트.
...
대담한 중앙 분할선과 네온 수채화 배경을 가진 분할 화면 캐릭터 시트를 생성하기 위한 프롬프트.
...
여관에서 던전 도전에 대해 논의하는 모험가들을 대사와 효과음과 함께 담은 두 페이지 분량의 컬러 만화를 생성하기 위한 프롬프트입니다.
...
1960년대 기계 도면 스타일의 가정용 에스프레소 머신 기술 도해입니다.
...
### No. 25: AI 플랫폼을 위한 모던 SaaS 대시보드 UI
...
세련된 다크 모드 미학을 가진 AI 분석 플랫폼을 위한 고해상도 4K 모던 SaaS 대시보드 인터페이스를 생성하기 위한 프롬프트.
...
### No. 26: 다크 판타지 RPG 인벤토리 사용자 인터페이스
...
갑옷을 입은 성기사를 중심으로 한 고해상도 다크 판타지 RPG 인벤토리 인터페이스를 생성하기 위한 프롬프트입니다.
...
초밥 실루엣과 겹치지 않는 배경에 크고 오목하게 들어간 진홍색 워드마크가 있는 포스터를 위한 프롬프트입니다.
...
제품 카톤을 세 단계(고립된 주인공, 재료 컨텍스트, 완성된 요리 옆)로 시각화하라는 프롬프트입니다.
...
중앙에 위치한 제품 파우치를 그릇, 베리, 곡물, 잎사귀가 둘러싸고 있으며, 명확한 수직 통로가 있는 이미지를 위한 프롬프트입니다.
...
일관된 검정색 영역, 컵 스케일, 타이틀 구역, 마이크로카피, 황동 클립을 갖춘 컬렉터블 커피 메뉴 포스터 디자인을 위한 프롬프트입니다.
...
히어로 제품 튜브가 내부 평면을 넘어 돌출되어 있으며 접촉 그림자가 있는 3D 빌보드 시각 자료를 위한 프롬프트입니다.
...
### No. 32: 중앙 히어로 제품이 있는 세 개의 콘 라인업
...
중앙의 피스타치오 콘이 지배적이고 측면 콘들은 앵글을 이룬 프레이밍 요소로 배치된 세 가지 맛의 콘을 특징으로 하는 구도를 위한 프롬프트입니다.
...
로고, 헤드라인 칼럼, 배지, 제품 구성을 포함한 일관된 브랜드 레이아웃으로 구성된 다섯 개의 겹쳐진 패널 생성을 위한 프롬프트입니다.
...
각각 증기가 나는 검정색 그릇과 과장된 붓글씨체 요리 이름이 특징인 네 개의 수평 액트를 묘사한 포스터를 위한 프롬프트입니다.
...
### No. 35: 소스 흐름을 가로지르는 소셜 포스트 인터페이스
...
소스의 흐름을 수평으로 가로지르는 미니멀한 검정색 소셜 포스트 인터페이스 띠가 특징인 이미지를 위한 프롬프트입니다.
...
깨끗한 자연적인 면과 숨겨진 재료가 있는 어두운 면을 보여주는 수직 분할 포스터를 위한 프롬프트입니다.
...
두 개의 절제된 다크 초콜릿 액체 아치가 대칭적인 궤도 운동으로 제품을 감싸는 프리미엄 제품 포스터를 위한 프롬프트입니다.
...
포장된 두 개의 제품 바가 중심을 가로질러 서로 반대 방향으로 대각선으로 교차하는 모습을 묘사하는 프롬프트입니다.
...
전경에서 후경까지 대각선으로 배열된 세 개의 그릇 이미지를 위한 프롬프트입니다.
...
### No. 40: 스마트폰을 시각적 중심으로 한 인스타그램 여행 캠페인
...
현대적인 스마트폰을 시각적 중심으로 사용한 고급스러운 인스타그램 여행 광고를 제작하기 위한 프롬프트입니다.
...
### No. 41: 콜렉터블 피규어용 세로형 소셜 미디어 포스터
...
대담한 타이포그래피와 단색 노란색 배경을 사용한 고급 콜렉터블 피규어를 광고하는 고해상도 세로형 소셜 미디어 포스터를 위한 프롬프트입니다.
...
강력한 타이포그래피 계층 구조와 가독성을 갖춘 책 표지를 디자인하기 위한 프롬프트입니다.
...
제공된 논문과 규정을 바탕으로 2026년 신경과학 학회 포스터를 생성하기 위한 프롬프트입니다.
...
회색 스튜디오에 매달린 떠 있는 도시 디오라마 4개(2x2 그리드)를 생성하는 프롬프트입니다.
...
### No. 45: 짜증나는 주제에 대한 풍자적 시스템 포스터 그리드
...
짜증나는 주제에 대한 풍자적 에디토리얼 포스터 4개(2x2 그리드)를 위한 프롬프트입니다.
...
도시 랜드마크를 상세한 수제 종이 디오라마로 묘사한 프리미엄 세로형 여행 포스터를 생성하는 프롬프트입니다.
...
수채화 배경이 있는 스플릿 스크린 캐릭터 시트를 생성하는 프롬프트입니다.
...
바리스타 소피아의 드라마틱한 히어로 이미지와 강렬한 타이포그래피를 특징으로 하는 프리미엄 캐릭터 시트를 생성하는 프롬프트입니다.
...
"이봐, 들어봐!"라고 말하는 소녀와 그녀의 말풍선을 "못 들은 척할게"라는 생각 풍선으로 가리는 소년이 그려진, 세로 4:5 비율의 만화 스타일 이미지입니다.
...
"우리 싸우지 말자?"라고 말하는 소년과 그의 말풍선을 물어뜯는 사나워 보이는 소녀가 등장하는 만화 스타일 장면입니다.
...
서로 다른 시대의 네 가지 패션 운동을 16:9 에디토리얼 포스터 스타일로 시각화한 2x2 그리드를 생성하기 위한 프롬프트입니다.
...
네 가지 다른 정부 체계를 파스텔 스토리북 스타일로 시각화한 2x2 그리드 이미지를 생성하기 위한 프롬프트입니다.
...
고인이 된 네 명의 체스 전설을 위한 영웅적인 삽화 추모 포스터 2x2 그리드를 생성하기 위한 프롬프트입니다.
...
- [Atlas Cloud GPT Image 2.5 Flare text-to-image](https://www.atlascloud.ai/ko/models/openai/gpt-image-2.5-flare/text-to-image?utm_source=github&utm_campaign=awesome-gpt-image-2.5-prompts)
- [Atlas Cloud GPT Image 2.5 Flare edit](https://www.atlascloud.ai/ko/models/openai/gpt-image-2.5-flare/edit?utm_source=github&utm_campaign=awesome-gpt-image-2.5-prompts)
- [Atlas Cloud GPT Image 2.5 Sunburst text-to-image](https://www.atlascloud.ai/ko/models/openai/gpt-image-2.5-sunburst/text-to-image?utm_source=github&utm_campaign=awesome-gpt-image-2.5-prompts)
