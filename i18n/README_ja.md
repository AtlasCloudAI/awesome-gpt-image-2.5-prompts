...
ミントグリーンの室内プールの傍らに座る、小柄なスペイン人女性の、洗練された縦長の美しいポートレートです。
...
太陽に晒されて白くなった石のアーチに縁取られた、二人の女性の画像を作成するためのプロンプト。
...
同じ女性を異なる角度から撮影した4枚のスタジオ写真で構成されるキャラクターシートを生成するためのプロンプト。
...
参考キャラクターに基づき、異なる絵文字ステッカーを3x3グリッドで生成するプロンプト。薄いグラデーション背景に中国語と英語のテキストが配置されます。
...
### No. 7: ドットアートキャラクターの4種類の動きスプライトシート
...
4種類のドットアートキャラクターアニメーション（各最大4フレーム）を、一貫したデザインで維持したスプライトシートを生成するプロンプト。
...
スケッチを基に、その核心となる主題、感情、ジェスチャーを保ちながら詳細を再構築したオリジナルアートワークを作成するプロンプト。
...
赤みがかった茶髪の若い女性の、写真のようにリアルな朝のライフスタイルポートレート。
...
モダンなホームオフィスのソファでリラックスする若い女性の、スマートフォンで撮影したような写真のようにリアルな画像を生成するプロンプト。
...
反射する背景のある室内で撮影された、若い女性のリアルなiPhoneポートレートを生成するためのプロンプト。
...
キャラクターの戦闘アクション用に、透明背景、4x4レイアウトの簡素化された128ピクセルのピクセルアート・スプライトシートを生成するプロンプト。
...
子供の描いた絵を、まずはムーディな夜のキャンプサイトに変え、次に昼間のバージョンにし、最後にキャラクターの服をオーバーオールに変更する一連のプロンプト。
...
一筆書きの連続線で描かれた、縦長のミニマリスト風国別アートプリントを作成するためのプロンプトです。
...
人物の核となるアイデンティティと特徴を保ちながら、1980年代の外見を再構想するためのプロンプトです。
...
前面と背面のビューを示す、浮遊する2枚の特大Tシャツの3Dモックアップを生成するためのプロンプトです。
...
アップロードされた写真を顔の参考資料として使用し、ラグジュアリーメンズウェアの編集風コラージュを作成するためのプロンプトです。
...
アップロードされた画像を使用して、被写体の顔の特徴と自然な外見を保持するよう指示するプロンプトです。
...
アップロードされた写真を顔のアイデンティティと構図の参考に使用し、プロフェッショナルなスタジオポートレートを作成するためのプロンプトです。
...
顔のアイデンティティリファレンスを使用して、一貫した男性モデルが登場するファッションコラージュを作成するためのプロンプトです。
...
未来的な宇宙船のコックピット内にいる、自信に満ちた女性宇宙飛行士の写実的ポートレートを生成するためのプロンプトです。
...
アップロードされた画像を顔の参照として使用し、全身の超リアルなスタジオファッションポートレートを作成するためのプロンプトです。
...
### No. 23: シネマティックなバイクライフスタイルポートレート
...
プレミアムスポーツバイクの横で屋外に座る男性の、超リアルなポートレートです。
...
顔のアイデンティティ参照を使用して、男性の超リアルなエディトリアルポートレートを作成するためのAIプロンプトです。
...
### No. 25: 参照画像からのプレミアムファッションポートレート
...
顔のアイデンティティ参照を使用して、スタイリッシュな成人男性のプレミアムファッションポートレートを作成するためのプロンプトです。
...
参照画像に基づいて、女性の高品質で超写実的な自然なライフスタイルポートレートを生成するためのプロンプトです。
...
被写体の写真を顔の参照として使用し、リアルな屋外の冬のファッションポートレートを作成するためのAIプロンプトです。
...
スタイリッシュな成人男性の超リアルなラグジュアリースタジオポートレートを生成するためのAIプロンプト。
...
アップロードした画像を構図と雰囲気の参考として使用し、シネマティックな雨のポートレートコラージュを作成するためのプロンプト。
...
特定の女性の顔の特徴や外見を忠実に保存した、3:4縦長の超リアルなスマートフォン自撮りを生成するためのプロンプト。
...
### No. 31: シネマティックファッションポートレートのアイデンティティ
...
被写体のアイデンティティと外見を保存したクローズアップのファッションポートレートを生成するためのプロンプト。
...
顔のリファレンスを使用して、フォーマルな服装の若い男性のクローズアップスタジオポートレートを作成するためのプロンプト。
...
### No. 33: アイデンティティを保持したシネマティックなライフスタイル写真
...
特定の東アジア系女性のアイデンティティと特徴を保持しつつ、写真のようにリアルなシネマティックな画像を作成するプロンプトです。
...
### No. 34: 参照画像を用いたシネマティック・ファッション編集写真
...
暗い設定で撮影された女性のシネマティックなファッション写真を、参照画像からアイデンティティを保持して生成するプロンプトです。
...
アップロードされた画像を顔のアイデンティティ参照として使用し、成人男性の超高精細な編集ポートレートを作成する画像生成プロンプトです。
...
ChatGPT Astraを使用して、裏切り者の斧使いとのドラゴン戦闘の画像を生成するためのプロンプト。
...
ダイナミックでフォトリアルなアングルから英雄的巨大ロボットを描いた画像を作成するためのプロンプト。
...
自信に満ちた線画、最小限のセルシェーディング、明るく鮮やかな色彩を特徴とするアニメスタイルの画像を作成するためのプロンプト。
...
明確に読み取れるラベルを付けた2つの商品画像を、正面から見たフラットレイアウトスタイルで生成するためのプロンプト。
...
大きく広げた翼を持つ巨大で威厳のあるハクトウワシの前に中央に立つ女性を特徴とする、シネマティックなファンタジーシーンのプロンプト。
...
### No. 41: モデルのアイデンティティを保持したハイファッション広告
...
特定のモデルの顔の特徴を保持した、プレミアムで超現実的なハイファッション広告画像を作成するためのプロンプト。
...
添付された記事をもとに、日本の雑誌『アフタヌーン』のスタイルで4ページの漫画を生成するためのプロンプト。
...
### No. 43: プレミアムシネマティック・キャラクターリファレンスシート
...
厳格なビジュアルガイドラインを用いて、分割画面のキャラクターリファレンスシートを作成するためのプロンプト。
...
透明背景の、簡略化された128ピクセルのピクセルアート戦闘アニメーションスプライトシートを生成するためのプロンプト。
...
### No. 45: キャラクター戦闘スプライトシート（ピクセルアート）
...
キャラクターの戦闘モーション用の16フレームのピクセルアートスプライトシートを生成するためのプロンプト。
...
### No. 46: キャラクター戦闘スプライトシート（ピクセルアート）
...
キャラクターの戦闘モーション（待機、斬撃、防御、ノックダウン）を16フレームで表現した、128pxの簡素化ピクセルアートスプライトシートを作成するプロンプト。
...
被写体のアイデンティティを保持するため、参照写真を用いた全身の写実的ポートレートを作成するプロンプト。
...
厳格な視覚的ガイドラインを用いて、2人のキャラクターの分割画面形式リファレンスシートを生成するプロンプト。
...
2人のキャラクターのための、厳密なビジュアルリファレンスを使用した分割画面キャラクターシートを生成するプロンプト。
...
### No. 50: ドム＆ブライアン 分割画面キャラクター設定シート
...
アップロードされたリファレンスを使用して、2人のキャラクターの分割画面キャラクターシートを生成するプロンプト。
...
キャラクター「ベイビー」の、大胆なタイポグラフィと厳密なビジュアルリファレンス遵守を特徴とするプレミアムキャラクターシートを生成するプロンプト。
...
### No. 52: シネマティックキャラクター設定：シュミット＆ジェンコ
...
大胆なタイポグラフィと厳密なビジュアルリファレンスを備えたプレミアムキャラクターシートを作成するためのプロンプト。
...
厳密なビジュアルガイドラインに従った分割画面キャラクターリファレンスシートを生成するプロンプト。
...
厳密なビジュアルガイドラインを使用して、ネオとトリニティの分割画面キャラクターリファレンスシートを生成するプロンプト。
...
### No. 56: ケビン vs ウェット・バンディッツ キャラクター設定
...
厳密なビジュアルリファレンスを使用して、ケビン vs ウェット・バンディッツのプレミアムキャラクターシートを生成するプロンプト。
...
### No. 57: 夜の屋上階段 エディトリアルファッションショット
...
クールなシネマティックな雰囲気を持つ、夜の屋上階段でのハイパーリアリスティックなファッションエディトリアル写真のプロンプト。
...
厳密なビジュアルガイドラインを使用して、2人のキャラクターの分割画面キャラクターリファレンスシートを生成するプロンプト。
...
### No. 59: マイクとマーカスのプレミアムシネマティック・キャラクター・バイブルシート
...
厳格なビジュアルガイドラインを用いて、分割画面のキャラクターリファレンスシートを生成するためのプロンプト。
...
### No. 60: リーとカーターのプレミアムシネマティック・キャラクター・バイブルシート
...
厳格なビジュアルガイドを用いて、分割画面のキャラクターリファレンスシートを作成するためのプロンプト。
...
透明な台座付きで、コンピューターデスクの上に置かれたリアルな1/7スケールキャラクターフィギュアの画像生成プロンプト。
...
顔の同一性リファレンスを使用して、同一の成人男性の7つのバージョンを1枚の写真に収めて生成するAIプロンプト。
...
### No. 63: 顔の同一性リファレンスを用いたファッションポートレート
...
写真を顔の同一性リファレンスとして使用し、プレミアムな全身ファッションポートレートを生成するプロンプト。
...
被写体のアイデンティティと自然な特徴を保ちながら、現実的で夢想的な映画的な肖像画を作成するためのプロンプト。
...
ユーザーがアップロードした写真から、被写体のアイデンティティと似顔絵を保持した、詳細で可愛らしい3Dクレイメーション肖像画を生成するためのプロンプト。
...
遊び心のある日本のデザイン誌スタイルで、高解像度の縦長編集ポスターを作成するためのプロンプト。
...
居心地の良い寝室で遊ぶ幼児のアニメーションシーンを作成するためのプロンプト。
...
被写体の正確な顔の特徴とアイデンティティを保ちつつ、1980年代のレトロ・ヴィンテージスタイルでポートレートを生成するためのプロンプト。
...
提供された参照画像に基づいて、プロフェッショナルなキャラクターモデルシートを生成するためのプロンプト。
...
入力画像が持つ正確な視覚スタイルでレンダリングされた新しい被写体。
...
参考画像の衣服を被写体にフィットさせ、顔、ポーズ、照明は変更しない。
...
ある参考画像から取り出した被写体を、別の参考画像の光と背景に合わせて配置する。
...
レイアウト、比率、遠近法を保持しながら、フォトリアリスティックにレンダリングされた絵。
...
提供された人物を中心に構築された、意図的に非映画的な、写真のようなアクションシーン。
...
水彩画の絵本の主人公を、複数のイラストで同一性を保てるほど厳密に指定する。
...
ニューヨーク地下鉄内で、柔らかな顔立ちと乱れた黒髪の女性を捉えた、自然なライフスタイル写真のためのプロンプト。
...
同じ実在の女性を正面、斜め前、横顔、背面から見たビューを示すキャラクター参考資料シートを生成するためのプロンプト。
...
アップロードされた写真を、写真のリアリズムとミニマルな水彩ビネットスタイルを組み合わせた洗練されたアートワークに変換するためのプロンプト。
...
アップロードされた各写真を、結合せずに個別の高品質ポスターに変換するためのJSON指示です。
...
夜の書斎を生成し、その後、ノートパソコンの画面の内容のみを自動化フローチャートに置き換えるプロンプト。
...
画像を、台詞・ストーリー・キャラクター設定を保ちつつ見開き2ページのストーリー漫画風にリドローするプロンプト。
...
環境プレートを、特定の構図要素を保ちつつ、より自由で絵画的なスタイルに編集するプロンプト。
...
アップロードされた画像を参考資料として使用し、ひどく損傷した古い肖像画を、清潔で現代的な見た目の写真に修復するためのプロンプトです。
...
オブジェクトの元の形状、質感、素材を保ちながら、色をネオンブラックに変更するためのプロンプト。
...
上段を元の参照写真として保持したまま、縦に二つのパネルを積み重ねた単一の縦長画像を作成するプロンプトです。
...
メインの被写体を自然な位置に保ちながら、写真を選択したアスペクト比にリサイズするプロンプトです。
...
特定の角度から窓から差し込む柔らかくない直射日光で照らされ、顔に鋭い影を作り出すポートレートを描写するプロンプト。
...
キャラクターの簡略化された128ピクセルのバトルモーション・スプライトシートを、透明背景の4x4グリッドで作成するためのプロンプト。
...
雲と太陽の写真を、太陽を花の中心に、雲を蓮の花びらに見えるように加工し、元の画像を改変しないプロンプト。
...
テキストの言語のみを入れ替え、レイアウトの他のすべてのピクセルはそのままにした編集。
...
クリーンなアルファチャンネル上に切り抜かれたパックショット。輪郭は鮮明で、ハローはなく、ラベルの可読性は保たれている。
...
単一のオブジェクトのみを除去し、他は一切変更しないワンライン編集。
...
既存のモックアップの季節と時間帯を変更する、5語によるフォローアップ編集。
...
架空の次世代3Dオープンワールド恋愛RPGを主人公視点で捉えた、リアルなスクリーンショットを生成するためのプロンプトです。
...
有名な建築物を、柔らかなパステルカラーと手作り感のあるディテールで表現した、可愛らしくエレガントな3Dジオラマを生成するためのプロンプトです。
...
葉のガウンとアイボリーのパラソルを持つ、横顔の細身の女性を描いた、縦長の植物をモチーフにしたペーパーアート作品です。
...
家電量販店に多数の統一された箱と共に展示された、白と金のパラディンロボットのモデルキットの、写真のようにリアルな画像を生成するためのプロンプトです。
...
夜の歴史的な市壁のそばに立つ若い女性と、光を放つ伝統的な中国建築を写した、リアルな自然体の旅行写真。
...
1920年代から1930年代に着想を得た、レイヤードペーパー・レリーフとエンボス加工されたイラストスタイルの、落ち着いたカラーパレットの旅行ポスターを作成するためのプロンプト。
...
猫のひげ、蝶の複眼、そして少女の肌を写した、写真のようにリアルなマクロ接写。
...
文字盤のテキストがはっきり見える、高級チタン製クロノグラフ腕時計の、写真のようにリアルな雑誌広告モックアップ。
...
### No. 14: プレミアムコーヒーバッグのプロダクトモックアップ
...
山のラインアートが施されたマットブラックのコーヒーバッグを、素朴な木の上に配置した、商業用の写真のようにリアルなモックアップ。
...
秋の服装をまとった若い女性モデルを、ソフトなパステルカラーで描いた、清潔感のあるファッションルックブックポスターを生成するためのプロンプト。
...
有名なランドマークの小さな3Dモデルを特徴とする、愛らしい手作りのミニチュアシーンを生成するためのプロンプト。
...
黄昏時の都市を見下ろす、成人した東アジア系女性の、写真のようにリアルなエディトリアルポートレート。
...
### No. 18: 日差しの差し込むカフェでの女性のシネマティックポートレート
...
日差しの差し込む近所のカフェにいる、成人した東アジア系女性の、写真のようにリアルなシネマティックポートレートを作成するためのプロンプト。
...
### No. 19: 非対称エディトリアル・ハーフフェイスポートレート
...
被写体をフレーム左端に配置した、エディトリアルスタイルのポートレートを作成するためのプロンプト。
...
### No. 20: 場所をテーマにしたプレミアムエディトリアルアートポスター
...
指定された場所をテーマに、高品質でモダンなアートブックスタイルのポスターを作成するためのプロンプト。
...
夜の温かみのある普通のアパートメントの設定で、女性を撮影したリアルな16:9横長写真を生成するためのプロンプト。
...
一般的に嫌われがちな食材のみで構成されたデラックスパフェを、30種類の材料と注釈付きで生成するプロンプト。
...
ミッドセンチュリーの日本鉄道観光スタイルを採用した、プレミアムな縦長の旅行ポスターを作成するためのプロンプト。
...
電車が猛スピードで通り過ぎる中、静止する茶髪の若い女性の高精細ポートレート。
...
洗練されたメイクとキツネ顔の背が高くスリムなモデルが、チャイナドレスを着て、柔らかく夢のような照明の中で撮影されたハイアングルポートレート。
...
農家とポプラが立つ黄昏時の水浸しの平原を描いたシネマティックなフィルムスチールで、水面に不一致の反射が映るプロンプト。
...
16:9のアスペクト比で、ボーカロイドMVスタイルの様々なショットを3x3グリッドで作成するプロンプト。
...
飛行中、離陸、着陸時のカラスを16ビットスタイルで描いたスプライトシートを生成するためのプロンプトです。
...
岩山の道から見上げた、軍馬に乗った剣闘士の極端なローアングルショットです。
...
岩礁にズームインし、岩の上の光を観察し、正確に8本の腕を持つタコを含む画像を生成する指示です。
...
### No. 31: ボタンの中の女性たちのシネマティックアナモルフィック静止画
...
大きな淡いピンク色のボタンの中に座る、5人のミニチュアサイズの中国女性を捉えたワイドアナモルフィック写真です。
...
前景と背景を埋め尽くす、密集したピンクの花咲く枝に囲まれた女性の写真です。
...
夕日のビーチに座り、手の中の砂を見つめるロボットの、リアルでシネマティックなシーンです。
...
ネオンピンクとグリーンの毛皮のオオカミ耳とチョーカーを特徴とする、サイバーパンクスタイルのふわふわコスチュームを生成するプロンプトです。
...
そばかすと風になびく髪を持つ若い女性の、生のiPhone写真を生成するプロンプトです。
...
### No. 36: 1980年代風、ホワイトハウスの芝生でエイリアンを迎える大統領
...
1980年代スタイルの写真として、ホワイトハウスの芝生でエイリアンを迎える大統領を描いた画像を生成するプロンプトです。
...
豪華で架空の唐代の宮廷夜宴を描いた、ワイドスクリーンの歴史映画風スチール写真です。
...
黒いロングヘアの成人東アジア人女性を被写体としたリアルなファッションフォトのプロンプト。顔に温かい光と影が落ち、自然な肌の質感が表現されています。
...
### No. 39: ブラックミストフィルターをかけた台湾人女性のポートレート
...
ブラックミストフィルター、オーバーレイマスク、ソフトライトCCD写真技法を用い、繊細なハイライトを特徴とするポートレートのプロンプト。
...
雨上がりの川辺に、雨滴をまとった一本の赤い曼珠沙華（彼岸花）をリアルに描写する画像生成のプロンプト。
...
閉店後の時間帯を、濡れたウール、肌、街路灯に焦点を当てて、情緒的な雰囲気で描写する画像のプロンプト。
...
風に髪や衣装をなびかせた3人の人物を、強い太陽光と豊富なシルク素材を用いてスタイリングしたファッション・エディトリアルシーンのプロンプト。
...
駐車場に停めたテスラ モデルYを、0.5倍カメラで撮影したようなリアルなiPhone写真を生成するプロンプト。
...
黄昏時のエコ未来都市、巨大な地球が見える月面都市、銀河の熾天使守護騎士、氷晶の大聖堂で舞う銀髪の女王を描写するプロンプト。
...
100人以上の軍勢、キャラクター「ルナ」、白いドラゴン、日本語の看板、精巧な槍の扱い、雨の反射光を含む複雑なシーンのプロンプト。
...
ウサギが跳びはねる様子と、灰色のネザーランドドワーフ種のウサギが跳びはねる様子を生成するプロンプト。
...
カジュアルな服を着たアニメ風の若い男女が大きな公園にいる画像生成プロンプト。超高精細16K解像度で、コントラストと彩度を強調してレンダリング。
...
『紅楼夢』の一場面を、伝統的な中国の長巻絵画として再構築するプロンプト。
...
砂漠のブルータリズム様式で、有名人と建築家を融合させたモノリシックな土の建築物の画像を生成するプロンプト。
...
ゴールデンアワーに並木道を歩く女性の、スマートフォン撮影を想定した写真のように写実的な画像生成プロンプト。
...
サントリーニ、アマルフィ海岸、パリにインスパイアされた洗練されたラグジュアリー旅行ポスターコレクションを作成するプロンプト。
...
晴れた午後の静かな都市公園で、長いダークブラウンの髪をした若い女性の自然な屋外ポートレート。
...
致命的な砂漠の暗殺者をフィーチャーした、映画的なキャラクター参考資料シートを生成するプロンプト。
...
夜のプライベートラウンジにいる美しい東アジア人女性の、スマートフォン撮影を想定した超高写実的なファッションフォトのプロンプト。
...
国の象徴的なランドマークと文化的要素を特徴とするヴィンテージスタイルの切手を作成するプロンプト。
...
認識可能な建築的ランドマークをフィーチャーした、スタイライズされた手描き風ヴィンテージ旅行ポスターを作成するプロンプト。
...
現代的な地下駐車場にいる美しいユーラシア系女性の、超高写実的なポートレートのプロンプト。
...
陶器のプレートに盛られた特定の料理を題材にした、魅力的なミニチュアシーンを生成するためのプロンプトです。
...
有名な店舗やブランドを題材にした、幻想的な3D粘土製ミニチュアジオラマを生成するためのプロンプトです。
...
中心となる象徴的な構造物を特徴とする、詳細で居心地の良いミニチュアワールドを作成するためのプロンプトです。
...
白いスタジオ背景を前にした男性のクローズアップポートレートで、非対称な構図を特徴とします。
...
長い時間をかけて風食によって形成された、白い迷路の風景を生成するための描写プロンプトです。
...
夜の闇に佇む白竜騎士が、敵ではなくたった一つの小さな石に足を止める様子を、哲学的キャプションと共に描いたシーンです。
...
短い黒髪の自信に満ちた男性の、プレミアムなエディトリアルスタイルのスタジオポートレートを生成するためのプロンプトです。
...
温かみのあるシームレスなスタジオ環境で、自信に満ちた男性モデルの全身ポートレートを生成するためのプロンプトです。
...
アルプス、ドバイ、ヨーロッパのシャレー体験を融合させた、洗練されたラグジュアリーライフスタイルの旅行キャンペーンのためのプロンプトです。
...
くし、ヘアゴム、靴紐が仕事を終える頃、報告書を読むブルーマジックマスターの朝の情景を描いたシーンです。
...
白い土、塩、石灰岩の岸辺に船が到着し、その先には見えない土地が広がる情景を描写するためのプロンプトです。
...
海辺に住み、海の兆しを読む沿岸漁師であるエルフを描写するプロンプトです。
...
### No. 70: プロフェッショナルなスタジオヘッドショットポートレート
...
成人男性のプロフェッショナルなスタジオヘッドショットポートレートを生成するためのプロンプトです。
...
暗い木製の扉口と大きな鏡がある豪華なモダンインテリアに佇む、自信に満ちた男性の超リアルなライフスタイルポートレートです。
...
異世界の地図の前に、机の木目を思い出す槍使いの夜を詩的に描写したプロンプト。
...
ドラゴンの翼の下で休憩する2人のドラゴンナイト、地図と硬いパンが描かれた情景。
...
### No. 74: 居心地の良い夕方の図書館でくつろぐスタイリッシュな男性
...
夕方早くの高級図書館ラウンジでくつろぐ男性の超リアルなポートレート。
...
### No. 76: ワインレッドのフルボディエディトリアルポートレート
...
クリーンなスーツを着た自信に満ちた黒人男性が、ミニマリストなスタジオ設定で座っている超リアルなエディトリアルポートレート。
...
魔法が日光や石畳が彼女に届く前に消し去る、ブルーマージマスターの午後の情景。
...
薄い炎が地面の裂け目から立ち上り、遠くにカスピ海を望む黄昏の荒野の情景を描写したプロンプト。
...
ドラマチックな海の夕焼けシーンで、ヴィンテージの木造帆船を超高精細に描いたイメージです。
...
月光に照らされた竜の厩舎で、白竜騎士団長が眠る竜の左翼の負傷を注意深く調べるシーンです。
...
午後の火山都市で、遠くに山々を望みながら青い旅布を繕うシーンです。
...
最高の称号を失った槍使いが、鏡の世界の朝、濡れた靴紐から一日を始める様子を描いたプロンプトです。
...
彼女の肉体が必要とされなかった時代の、本と椅子と紅茶を前にした夜明け前の青魔道士ルナを描くシーンです。
...
乾いた荒野で地から炎が漏れ出し、灰色の泥が流れ、遠くにカスピ海を望む風景を描写するプロンプトです。
...
夜、隣でエル・ドラゴが眠る中、槍のタコを確認する黒竜騎士ルナを描いたシーンです。
...
キャラクターのフェルンとフリーレンが会話している様子を描いた、健全なイラストです。
...
緑が褪せ、オーカー、錆色、灰白色、薄緑の層が露わになった、風と雨に削られた丘の風景を詩的に描写するプロンプトです。
...
白壁と竹影を背景に、団扇を持つ古典的な女性を描いた、東洋の美意識に基づく壁紙を生成するためのAIプロンプトです。
...
東京警察の重機ロボットチームが救命ボートを牽引して避難者を救助する様子を描いたプロンプトです。
...
天文観測デッキでの都市探索をテーマにした、明るい昼間のCCDスタイルのライフスタイル写真を生成するためのプロンプトです。
...
若い女性のリアルで自然なiPhoneスタイルのポートレートを生成するためのプロンプトです。
...
翡翠色の花窓のそばに立つ古典的な女性を描いた、優雅でミニマルな東洋風ポスターを生成するための画像プロンプトです。
...
### No. 93: 東アジア人女性のフォトリアリスティックな編集ポートレート
...
特定の顔の特徴を持つ若い東アジア人女性の、膝上からアイレベルでのフォトリアリスティックなポートレートを詳細に生成するためのプロンプトです。
...
河川をせき止めているゴミを清掃する警察重機ユニットのロボット部隊を描写したプロンプトです。
...
### No. 95: ハートオーバルフェイスのフォトリアリスティック編集ポートレート
...
特定の顔の特徴を持つ若い東アジア人女性の、膝上からアイレベルでの編集ポートレートを詳細に生成するためのプロンプトです。
...
20歳から26歳の女性の、iPhoneのメインカメラで撮影されたようなリアルでカジュアルな9:16縦向き写真を生成するためのプロンプトです。
...
水辺に佇む女性を、ミニマルで清涼感のあるスタイルで描いた、東洋の美意識に基づく壁紙を生成するためのプロンプトです。
...
特定の顔の特徴を持つ若い東アジア人女性の、フォトリアリスティックな編集ポートレート（カウボーイショット）を詳細に生成するためのプロンプトです。
...
都市的で芸術的な設定と特定の服装を特徴とする、明るい昼間のCCDスタイルのライフスタイル写真を生成するためのプロンプトです。
...
### No. 100: 東アジア人女性のフォトリアリスティック編集ポートレート
...
特定の顔の特徴を持つ若い東アジア人女性の、膝上からウエストレベルでの編集ポートレートを詳細に生成するためのプロンプトです。
...
桜が舞う青い門をくぐる優雅な古風な女性を描いた、東洋のミニマルな壁紙を生成するためのプロンプトです。
...
集中豪雨で水没した道路から車両を移動させる、重機対策隊の警察ロボットチームの画像。
...
アーバンスポーツ環境での、明るい日中に撮影されたCCDスタイルのライフスタイル写真を生成するためのプロンプト。
...
モダンな東洋女性の美学と朝の雰囲気を持つ、ミニマリストで明るく清潔な9:16縦長カバーを作成するためのプロンプト。
...
丸窓のそばでリラックスする女性を描いた、東洋的で禅的なミニマリストポスターを生成するためのプロンプト。
...
暴走トラックを止めようとする、重機対策隊の警察ロボット部隊の画像。
...
フープイヤリングを着けた、日本人または韓国人の若い女性の、全身で目の高さのエディトリアルポートレートのためのプロンプト。
...
### No. 108: メガネをかけた写真のようにリアルなウエストアップポートレート
...
太い白縁の大胆なメガネをかけた、日本人または韓国人の若い女性の、膝の高さからのウエストアップビューポートレート。
...
### No. 109: 女性と風鈴のある禅的ミニマリストカバーポスター
...
窓辺で風鈴に触れる女性を、ミニマリストで東洋的禅スタイルで描いた画像を生成するためのプロンプト。
...
緑の植生に覆われた、中国・深圳のポストアポカリプス都市の詳細な写真。
...
静かな夜の環境で、タヌキのような顔を持つ架空の若い日本人女性の、写真のようにリアルな画像を生成するためのプロンプト。
...
女性専用の賑やかな水族館を訪れる女性たちを、イルカの視点から360度ガラスタンク越しに見たリアルな画像。
...
### No. 113: ポリマークレイ製ブランドストアのミニチュア情景
...
居心地の良い歩道のディテールを備えたブランドストアのミニチュア粘土模型を作成するためのプロンプト。
...
コバルトブルーの空を背景に鮮やかな服を着た6人の人物を、ローアングルから撮影した架空ブランドのファッション広告キャンペーン。
...
1990年代後半の日本の手描きアニメーション映画の一場面で、刀を持った女性が桜吹雪の中で剣士と対峙する情景。
...
暗くロマンチックなエレガンスを備えたゴシックハロウィンホラーブライドの衣装のためのプロンプト。
...
### No. 117: 東アジア人女性のムーディーなシネマティックポートレート
...
白い肌と長い黒髪を持つ若い東アジア人女性の、ハイコントラストでシネマティックなクローズアップポートレートのためのプロンプト。
...
### No. 118: 濡れた横断歩道でのシネマティックな夜のストリートポートレート
...
夜の濡れた横断歩道上に立つ若い東アジア人女性のシネマティックなポートレートのためのプロンプト。
...
漆黒の髪と輝く黄金の瞳を持つ若い東アジア人女性の、シネマティックなクローズアップポートレートのためのプロンプト。
...
### No. 120: メガネとウインクのフォトリアリスティックなポートレート
...
メガネをかけ、ウェーブのかかった髪の若い東アジア系女性が優しくウインクしている、写真のようにリアルなポートレート。
...
乱れた黒髪の若い東アジア系女性がカメラを見つめる、写真のようにリアルなアナログフィルム風ポートレート。
...
静かな水の中に浮かび、髪を濡らした穏やかな若い女性のシネマティックなポートレート。
...
### No. 123: サイド編み込みのスタジオ・ビューティーポートレート
...
輝くような肌、長い黒髪、そして目を閉じた若い東アジア系女性の、写真のようにリアルなスタジオ・ビューティーポートレート。
...
車内にいる、薄いベージュのヒジャブを着用した若い東アジア系女性の、写真のようにリアルなクローズアップポートレート。
...
木製のテーブルで薄いヒジャブを着た若い女性がセルフィーを撮っている画像を生成するためのプロンプト。
...
振り返っている若い東アジア系女性の、写真のようにリアルなポートレートを生成するための詳細なプロンプト。
...
メガネをかけ、ウェーブのかかった髪と落ち着いた表情の若い東アジア系女性のポートレート。
...
陶器のような肌、アーモンド形の目、鮮やかな口紅をした若い東アジア系女性の、写真のようにリアルなポートレート。
...
露を含んだような肌と濡れた黒髪で振り返る若い女性の、写真のようにリアルなポートレート。
...
黒い背景を前に、長くてダークなウェーブのかかった髪の若い東アジア系女性のクローズアップポートレート。
...
長いウェーブのかかった髪、丸メガネ、柔らかな表情の若い東アジア系女性の、写真のようにリアルなポートレート。
...
### No. 132: ハニーハイライトのシネマティックなポートレート
...
ウェーブのかかった明るい茶色の髪と落ち着いた表情の若い東アジア系女性の、シネマティックなクローズアップポートレートのためのプロンプト。
...
### No. 133: ボブカットのフォトリアリスティックなポートレート
...
ショートの黒いボブカット、スモーキーアイシャドウ、輝く肌の若い東アジア系女性の、写真のようにリアルなクローズアップポートレート。
...
白いヒジャブと黒い野球帽をかぶり、穏やかな表情をした色白の若い女性のクローズアップ肖像画。
...
ダスティトープ色のシフォンヒジャブを着けた、色白で繊細な顔立ちの東アジア系若い女性のクローズアップセルフィー肖像画。
...
### No. 136: 涙を流す東アジア人女性のシネマティックポートレート
...
光沢があり涙で濡れた肌と、ふわふわした前髪を持つ若い女性の、写真のようにリアルなクローズアップ肖像画。
...
黒いボブヘアで色白の若い女性のクローズアップ肖像画。片方の赤い目が見え、もう片方は宝石が散りばめられた黒いレースのアイパッチで覆われている。
...
長い黒髪を持ち、カメラをまっすぐに見つめる若い女性のクローズアップシネマティックポートレート。
...
色白の肌と長い黒髪を持つ若い女性の肖像画を生成するための詳細なプロンプト。
...
乱れた黒髪を持ち、目を細めて微笑み、ほてって汗ばんだ肌の東アジア系若い女性のクローズアップ肖像画。
...
特定の顔の特徴とメイクを持つ東南アジア系若い女性のクローズアップ肖像画を生成するための詳細なプロンプト。
...
目を閉じ、リラックスした表情の東アジア系若い女性のクローズアップセルフィー肖像画のプロンプト。
...
色白の肌、ソフトなメイク、優しく友好的な表情を持つ、東アジア系またはインドネシア系の若い女性の肖像画を生成するためのプロンプト。
...
特定の特徴とスタイリングを持つ東アジア系若い女性の、写真のようにリアルなセルフィー肖像画を生成するためのプロンプト。
...
人間の観察者の視点から、ブラックホールに地球が飲み込まれる視覚的な段階を描いた4枚の絵を生成するためのプロンプト。
...
サイバーパンクの刀使い戦士と巨大な機械のボスエンティティという2体のキャラクターを描写するプロンプト。
...
荘厳な中国の天女が儀式用の漢服をまとい、バラ色に輝く雲を抜けて天上の宮殿の前に降臨する画像。
...
象牙色の漢服をまとった天女が朝の光に照らされ、流れるようなリボンと金色の簪をあしらった画像。
...
シルクの漢服をまとった穏やかな中国の天女が、ターコイズブルーの池のほとりで蓮の花に触れている画像。
...
白い玉の階段の上に立つ優雅な中国の天女が、刺繍を施した漢服をまとい、絹のリボンを漂わせている画像。
...
ノウ仮面とろうそくがある暗い古代中国の儀式祠堂にいる、東アジア系成人女性の写実的で映画的なポートレート。
...
濡れた黒髪とスモーキーなメイクをした東アジア系女性が、暗い古代の祠堂にいる写実的で映画的なポートレート。
...
夜間の儀式祭りで、風化したノウ仮面の隣に立つ東アジア系成人女性のクローズアップポートレート。
...
鋭い縁のグラスに注がれ、濃厚でカスケード状の泡冠を持つビールの写実的な画像を生成するためのプロンプト。
...
多孔質の豆腐の立方体が鍋の上に持ち上げられ、一滴の琥珀色のスープが垂れ下がっている画像を描写するプロンプト。
...
はっきりとした赤唐辛子の立方体、茶色がかった緑野菜、鮮やかな緑のピーマン、淡色の豆、透き通った琥珀赤の油が特徴の、スプーン一杯のチリレリッシュの画像。
...
### No. 157: リビングルームの女性クローズアップポートレート
...
居心地の良いモダンなリビングルームで、遊び心のある表情を浮かべる若い女性の超リアルなクローズアップポートレート。
...
夜の薄暗い寝室で、寝具の上に横たわる若い女性の写真のようにリアルな縦向きポートレートのプロンプト。
...
暖かい昼光に柔らかく照らされた背の高い草原で休む、若い東アジア人女性のフィルム風画像。
...
風がそよぐ晴れた日、明るいブルーの港辺でカジュアルにしゃがむ若い東アジア人女性のフィルムグレイン風ポートレート。
...
パリ風のショッピングストリートに立つ、アジア系の若い女性の高解像度・自然光によるストリートポートレートのプロンプト。
...
夜の電車の窓際に座る、成人した東アジア人女性の写真のようにリアルな、自然な表情のポートレート。
...
時計の背後に円形の氷と煙の衝撃波を配し、時計のシルエットはクリーンに保った画像。
...
異なる高さの段に商品を配置して奥行きを創造した、プレミアムベーカリーのディスプレイ画像を生成するプロンプト。
...
### No. 165: 柔らかな光の寝室でのスリップドレス姿のポートレート
...
柔らかく照明された寝室で、アイボリーのサテンスリップドレスを着た成人東アジア人女性の写真のようにリアルなポートレート。
...
強力な車が後方に巨大な砂煙を巻き上げながら、前景では清潔で静かなままであるイメージです。
...
巨大なオレンジ色の天体の炎の雲に逆光で照らされた魔女のシルエットのイメージです。
...
### No. 168: コシャリのボウルから立ち昇るトマトソースのハート
...
コシャリのボウルの上で、光沢のあるトマトソースがハート型を形成しているイメージです。
...
手作りの麺の糸から彫刻され、認識可能なシルエットを持つチェスの駒を生成するためのプロンプトです。
...
### No. 170: ボクシングリングで勝利を収めるゴールデンの腕を持つ特大ごま油ボトル
...
ボクシングリングに立ち、勝利を収めているゴールデンのボクサーの腕を持つ特大のごま油ボトルのイメージです。
...
### No. 171: コンパクトなモダンアパートのキッチンカウンターの情景
...
朝食カウンター、連続したカウンタートップ、ステンレスシンク、コンロ、フライパンを備えたコンパクトなモダンアパートのキッチンのイメージを生成するためのプロンプトです。
...
### No. 172: ロケットのように上向きに角度をつけたシャワルマのラップ
...
ロケットのように角度をつけ、開いた具材を焦点として強調したシャワルマのラップのイメージを生成するためのプロンプトです。
...
走っているバナナのマスコットが、特大の冷たいアルミ缶をカメラに向かって前に持っているイメージです。
...
特定の特徴と照明を持つ東アジア系女性のリアルなキャンディッド写真を生成するためのプロンプトです。
...
### No. 175: ミニチュアスケート場として盛り上がるクッキーのクリーム層
...
バニラクリーム層が滑らかなミニチュアスケート場として盛り上がり、スケールを冬の世界に変えるクッキーのイメージです。
...
### No. 176: 積み重ねたクリスプで作られたジェットコースターのレール
...
完璧に整列して積み重ねられたクリスプで構築され、フルループを特徴とするジェットコースターのレールのイメージです。
...
### No. 177: ヒーローバケットの背後にある赤いスポットライトゾーン
...
ヒーローバケットの背後に、二次的なライフスタイルの瞬間を映し出す3つのクリーンな赤いスポットライトゾーンを描写するイメージプロンプトです。
...
単一の光沢ある麺の流れを中心の垂直要素として特徴とするミニマリストなポスターのコンセプトです。
...
### No. 179: シネマティックな2.5Dガッシュアニメーション
...
筆のテクスチャとフラットなカラーブロックが目立つ、絵画的なガッシュコンセプトアートスタイルのシネマティックな2.5Dアニメーションを作成するためのプロンプトです。
...
### No. 180: 闇の中のムーディーなシネマティック・ポートレート
...
影から現れる髪の乱れた男性のシネマティックなポートレート。ドラマチックな照明と超現実的な質感が特徴。
...
自由の女神像の隣に巨人として立つ、ハンサムな若い男性を描いた、超現実的なシネマティックな旅行写真。
...
### No. 182: 素朴なポーチでのゴールデンアワーのポートレート
...
ゴールデンアワーに素朴なポーチでお茶を楽しむ若い男性の、温かみのある自然なライフスタイルポートレート。
...
セージブラシと松の木が生える緑の草原に立つ、カウボーイハットをかぶった女性の超現実的なポートレート。
...
特定の特徴を持つ若い東アジア系女性の、縦位置の自然な写真を生成するためのプロンプト。
...
デザートをテーマにした世界の中で、裂けた包装紙が口を開けたような効果を生み出す、対角線上に配置されたパッケージデザートバーの画像を生成するプロンプト。
...
縦長の楕円形の鏡に反射した女性の美のポートレート。マットなコバルトリップスティックが特徴。
...
砂とビールが渦巻く巨大なビールジョッキが砂漠から出現する、シュールなイメージ。
...
右下前景に大きく、シャープで、部分的に切り取られた本物のビール瓶を特徴とする画像のプロンプト。
...
### No. 189: モンドリアンの絵画をデスクトップ収納キャビネットに
...
モンドリアンの『赤、青、黄のコンポジション』を、色付きの引き出しを持つ機能的なデスクトップ収納キャビネットに変えるためのプロンプト。
...
想像上のシュールな主題を、非常にリアルな画像として生成するためのプロンプト。
...
ミカンの中に住む極小の人間たちのイメージ。果肉の房が建築物を形成している。
...
漆黒の髪と血のように赤い唇を持ち、鋭い横顔をした威圧的な人物の画像を生成するためのプロンプト。
...
若いカレンジンモデルをフィーチャーしたデートアプリの広告を生成するためのプロンプト。
...
夜明け前、ラジオを持った風化した顔の人物が屋上にいるポートレートを生成するためのプロンプト。
...
京都の伏見稲荷大社の象徴的な朱色の鳥居を、旅行キャンペーン画像としてスタイリングした、静かで早朝の情景を生成するためのプロンプト。
...
ユーザーの過去のAIアート嗜好に基づいて、その内面世界を反映したイラストを生成するリクエストプロンプト。
...
発光する都市が内包された、部屋サイズの透明なプロセッサのマクロショットを生成するプロンプト。
...
言語、コード、創造性が光によって相互接続された象徴的自画像を描写するプロンプト。
...
地中海風の建築とレトロフューチャリズムの要素を備えた、異世界における人類居住地の画像を生成するプロンプト。
...
氷の女帝であり君主であるツァリーツァを、様々な威厳と悲しみに満ちた称号と共に描写した肖像画を生成するプロンプト。
...
誰も到着する前から湯気が立ち昇り、一番早い電車が思考の中にあるような、朝霧に包まれたバス停の詩的なイメージ。
...
### No. 202: クラゲと宇宙天気予報のある夜のコインランドリー
...
外にはクラゲが浮かび、少女が宇宙天気予報を確認する夜間のコインランドリーの情景。
...
重ね着の鎧とローブを纏い、長剣を構える、背が高く優雅な東アジア系女性戦士の9:16縦長ポートレートを生成するプロンプト。
...
9:16の画像プロンプト。田舎の風景の中で、レトロな電話の美学で撮影された若い東アジア系女性を描写する。
...
静寂な伝統的な庭園に座る、新しい中国風チャイナドレスを着た若い東アジア系女性二人の、ハイパーリアリスティックな写真を生成するプロンプト。
...
9:16縦長、リアルなクローズアップセルフィーのプロンプト。クールで夢見がちな、ハイエンドなグレーのライフスタイルスタイルの若い東アジア系ダンサーを描写する。
...
### No. 207: K-POPコンサートステージ エディトリアルフォトグラフィ
...
コンサートでの20歳のK-POP女性アイドルの、シネマティックでエディトリアルスタイルの写真のプロンプト。
...
古典的な石の欄干にもたれかかる、美しい成人した東アジア人女性の、9:16縦長ファッションフォトのためのプロンプト。
...
輝くような空とそびえ立つ雲に焦点を当てた、詳細でロマンチックなファインアート風景画を生成するためのプロンプト。
...
カカリコ村を通り、ふわふわのパペットチキンの怒った群れに追いかけられるリンクの画像。
...
旅行キャンペーンのようなスタイルで、霧のかかった静かな朝のバリの棚田の情景を描く画像プロンプト。
...
### No. 212: ジャンプするプリンス・オブ・ペルシャのピクセルアート
...
コバルトディザーモニュメンタリズムのスタイルで、オリジナルのプリンス・オブ・ペルシャのキャラクターがプラットフォームを飛び越える様子を描く画像を生成するためのプロンプト。
...
冷たい夜霧の中、生と死の境界に立ち、静かな別れのための導きとして手を差し伸べる、死を司る少女を詩的に描写したプロンプト。
...
夜が黒に沈む中、金だけが護符の存在を照らし出す情景で、「夜歩き」「悪霊退散」の文字が特徴。
...
生と死の境界に立つ非人間の少女が、花嫁になる夢を見て、自らの姿を映し出す様子を詩的に描写したプロンプト。
...
恐れられる存在、ふぅふぅが、雨の日に古い部屋で本を読み、迷い込んだ小鳥がいる情景。
...
### No. 217: キツネキャラクターのピクセルアートスプライトシート
...
星を集める小さなキツネ、ノクサというキャラクターの、洗練されたピクセルアートアニメーションスプライトシートを生成するプロンプト。
...
### No. 218: Apple折りたたみスマートフォンコンセプトインフォグラフィック
...
同社の特徴的なクリーンなスタイルで、Appleの折りたたみスマートフォンの写実的なインフォグラフィックを生成するプロンプト。
...
柔らかい照明と細部まで描き込まれた肌の質感を持つ、若い東アジア女性の写実的なクローズアップ肖像画。
...
車内から見た、夕暮れ時の海岸道路のそばに立つ若いアジア人女性の写実的なスナップ写真。
...
ハスの葉を島に見立て、漢服を着た3人の小さな人々が住む様子を、水面からの映画的なマクロショットで描くためのプロンプト。
...
日本刀のように研ぎ澄まされ、磨き上げられた刃を持つ、三又の巨大な両手武器を描くためのプロンプト。
...
朝の光、雑誌の表紙のような肌、隠さないほど薄いガラスを持つ、季節の使者を詩的に描写するプロンプト。
...
渦巻く黒髪の上に幾何学的な金の円盤の冠を戴き、鋭い横顔で描かれた威厳ある人物の画像プロンプト。
...
大きなダイヤモンドのティアラを身に着け、鋭い横顔で描かれた輝く人物のAI画像プロンプト。
...
極端な広角視点で捉えた、若い東アジア女性の武侠スタイルの超写実的肖像画。
...
温かい映画的なトーンで、リラックスした家庭の雰囲気の中にいる若い東アジア女性の、9:16縦長の全身超写実写真のプロンプト。
...
様々なポーズをとるちびキャラクターの4x4グリッド・スプライトシートを生成するためのプロンプト。
...
### No. 229: 寝室でのフォトリアリスティックなスマートフォン自撮り
...
夜、ベッドでくつろぐ若い東アジア系女性の、フォトリアリスティックな風景スマートフォン自撮り。
...
国の境界線が、その象徴的なランドマークや文化的シンボルで構成された3Dミニチュア地図を生成するためのプロンプト。
...
エッフェル塔近くを歩く東南アジア系ヒジャブ女性の、超リアルなシネマティックな写真のためのプロンプト。
...
### No. 232: 高級ファッションファブリックで再構築された動物たち
...
4つの象徴的な高級ファッションブランドの特徴的な生地を使って再構築された動物たちの2x2グリッドを生成するためのプロンプト。
...
Appleの特徴的なスタイルで、ハイエンドスマートフォンのプロモーションポスターを生成するためのプロンプト。
...
ヴィンテージのスクリーンプリントスタイルと建築的なポータル構図を用いた、プレミアムな旅行ポスターを作成するためのプロンプト。
...
### No. 235: ストップモーション・クレイアニメーションのヒトカゲの孵化
...
粘土スタイルのヒトカゲが卵から孵化し、火を吹くストップモーションアニメーションを作成するためのプロンプト。
...
マグリットの『光の帝国』において浮かぶリンゴが、リンゴ型の紙のメモの山に置き換えられた創造的な再解釈。
...
### No. 237: 日本の家屋でのハイパーリアリスティックなファッションポートレート
...
伝統的な日本の木造廊下を歩く女性の、ハイパーリアリスティックなファッションポートレート。
...
壮大なローマ神殿、水道橋、遺跡を温かな黄金色のトーンで描いた、ウルトラミニマルな壁紙。
...
降りしきる雨、到着する電車、そして都市の上空を漂う鯉を描いた、架空のアニメのワンシーンフレームを生成するプロンプト。
...
様々な出来事に巻き込まれた末に、結局猫と友達になってしまう人物を描いた情景。
...
高原にある水族館の中で、眠たげな目をしたキツネとクラゲを、粒状の光の粒子を使わないスタイルで描いた画像。
...
着物を着た女性が、牛タンと麦飯の食事をしている情景を描いた画像。
...
### No. 243: 居心地の良い夜のリビングでくつろぐ女性のスナップ写真
...
居心地の良い夜のリビングで、若い女性がくつろいでいる様子を写実的に捉えたスナップ写真のためのプロンプト。
...
ジラメイクをしたチューニバイョウ（中二病）の女性が、肩出しトップスとコルセットロングドレスを合わせたゴシックロリータ衣装を着たアニメ風画像。
...
元カレが自分を去った理由が彼の死期が近いことだと知り、最期まで彼のそばにいることを決意する女性を描いたプロンプト。
...
幾何学的な制服、氷のように青い片目、角張った肩、赤い唇を持つキャラクター「リアンナ」を詩的に描写したプロンプト。
...
藍染め、木枠、石橋、水面に降り注ぐ朝の光を特徴とする、江南地方の染め工房の情景を描く画像プロンプト。
...
白い肌にそばかす、青い目、白金色の編み込み髪を持つ若い女性の、16:9比率の写実的な壁紙用ポートレート。
...
### No. 249: 船員のフォトリアリスティックな自然なポートレート
...
網を調整する年老いた船員の35mmスタイルの自然な写真。肌や素材の質感がリアルに表現されている。
...
1969年の特定の日付に、ニューヨーク州ベセルで撮影された、フォトリアルな屋外の群衆。時代考証が正確に行われている。
...
### No. 251: テディベア付きノスタルジックなクリスマスカード
...
思い出箱のような情景と、カードの文言を一字一句そのまま一行加えて構築された、印刷品質のホリデーカード。
...
### No. 252: ブリスター包装のヴィンテージおもちゃ飛行機コレクタブル
...
一字一句そのままの包装文言一行付きで、ブリスター包装された小売りスタイルのコレクタブルショット。
...
柔らかな雲が郵便局に変身し、制服を着た小さな動物たちが手紙を仕分けする幻想的なシーン。柔らかく癒される絵本スタイルで描かれています。
...
### No. 254: オートバイ上のハイパーリアリスティックなシネマティックポートレート
...
カスタムオートバイに乗った、スタイリッシュな顎鬚の男性のハイパーリアリスティックなシネマティックポートレート。
...
### No. 255: ハイファッションな白黒エディトリアルポートレート
...
巻き毛で重ね着をした若い男性の、白黒のハイファッションポートレートを作成するためのプロンプト。
...
クリーム色の紙の上に、エレガントなゴールドとネイビーのラインで描かれた、正方形フォーマットのアール・デコ調ポスターイラストです。
...
ダークな背景に、金色の輪郭と光沢のある表面を持つ、プレミアムなエナメルピン8点のコレクションを作成するためのプロンプト。
...
特定のラベルとUI要素を持つ、ダークテーマのSaaS分析ダッシュボードインターフェースを生成するためのプロンプト。
...
### No. 9: クリエイティブデザイン会社のウェブサイトスクリーンショット
...
クリエイティブデザイン会社の中国語ウェブサイト向けに、正面から見た、遠近法のないデスクトップ版ウェブページ全体のスクリーンショットを生成するためのプロンプト。
...
宋王朝の閨房で宋玉の着付けの全過程を描いた、プロフェッショナルでハイエンドな古代中国風ファッション写真シリーズを、厳密な3x3グリッドに配置して生成するためのプロンプト。
...
15時32分を示すアナログ時計、グリルで焼かれた黄金色のプロボローネチーズのハンバーガー、そしてグラスを描いた画像を生成するプロンプトです。
...
青と白を基調とした正方形のSNS画像で、スマートフォン1台、画像カード3枚、日本語タイトル、広い余白を含み、追加テキストはありません。
...
スペイン語の物理学の本に向けて、電磁気学に関する章を生成するプロンプトです。
...
宇宙開発をテーマにした、縦長の白黒1ページ漫画を生成するプロンプトです。
...
透明なガラスキューブの等角投影図。左上奥に赤いボール、手前右下に青いボールが入っており、キューブの上には緑のボールが浮かび、左下から右上へ黄色い柱が貫通しています。
...
モデルリストを掲載したタイポグラフィックなポスターと、マストヘッド（誌名）とカバーライン（見出し）を備えた雑誌の表紙を生成するプロンプト。
...
ダークエメラルド色のスタジオで大きな時計をフィーチャーした、縦向きの高級広告のプロンプト。
...
構図ラインと特定のレイアウトを含む、絵画的なキャラクターデザイン参考資料シートを生成するプロンプト。
...
二人のオリジナルキャラクターのための、プレミアムなスプリットスクリーンキャラクターシートを生成するプロンプトです。
...
男性の3層構成を特徴とする、超リアルなコーポレートポートレートコラージュ。
...
オリジナルの写真と、同じシーンの手描きイラストを組み合わせた縦長の画像を作成するためのプロンプト。
...
大胆な中央仕切りとネオンの水彩背景を持つ、分割画面キャラクターシートを作成するためのプロンプトです。
...
酒場でダンジョン攻略について議論する冒険者たちを、台詞と効果音付きで描いた2ページカラー漫画を生成するためのプロンプト。
...
1960年代の機械製図スタイルで描かれた家庭用エスプレッソマシンの技術図面。
...
### No. 25: AIプラットフォーム向けモダンSaaSダッシュボードUI
...
洗練されたダークモードの美学を持つAI分析プラットフォーム向けの、高解像度4KモダンSaaSダッシュボードインターフェースを生成するためのプロンプト。
...
### No. 26: ダークファンタジーRPGインベントリーユーザーインターフェース
...
鎧をまとった聖騎士を特徴とする、高解像度のダークファンタジーRPGインベントリーインターフェースを生成するためのプロンプト。
...
背景に大きな凹み加工の深紅色ロゴがあり、寿司のシルエットと重ならないポスターを作成するためのプロンプト。
...
製品の段ボール箱を、単体のヒーローショット、原材料のコンテキスト、完成した料理の隣という三つの段階で視覚化するプロンプト。
...
中央に商品パウチを配置し、周囲をボウル、ベリー、穀物、葉で囲んだ画像のプロンプト。明確な垂直の空間（コリドー）を設ける。
...
### No. 30: 同一デザインのコーヒーメニューポスターのグリッド
...
統一された黒いフィールド、カップのスケール、タイトルゾーン、ミニコピー、真鍮製クリップを持つ、コレクタブルなコーヒーメニューポスターのデザインプロンプト。
...
メイン製品のタブ（容器）が内側の平面を超えて投影され、接触影を持つ3Dビルボードビジュアルのプロンプト。
...
3つのフレーバーコーンを配置した構図のプロンプト。中央のピスタチオコーンが支配的で、両側のコーンは傾けてフレーミング要素とする。
...
ロゴ、見出しコラム、バッジ、製品構成を含む一貫したブランドレイアウトでデザインされた、5段に積み重ねられたパネルを生成するプロンプト。
...
4つの水平なシーン（アクト）を描いたポスターのプロンプト。各シーンは湯気の立つ黒いボウルと、特大の筆記体の料理名を特徴とする。
...
### No. 35: ソースの流れを横切るソーシャル投稿インターフェース
...
ソースの流れを水平に横切るミニマルな黒のソーシャル投稿インターフェースの帯を特徴とするイメージのプロンプトです。
...
清潔で自然な側面と、隠された要素を持つ暗い側面を垂直分割で示すポスターのためのプロンプト。
...
対称的な軌道運動で製品をフレーミングする、2つの抑制されたダークチョコレートの液体アーチを特徴とするプレミアム製品ポスターのプロンプト。
...
中央を反対方向に対角線上に交差する、2本のパッケージ製品バーを描写するプロンプト。
...
前景から背景へ対角線上に配置された3つのボウルの画像のためのプロンプト。
...
### No. 40: スマートフォンを視覚的アンカーとしたInstagram旅行キャンペーン
...
モダンスマートフォンを視覚的アンカーとして使用した、高級感のあるInstagram旅行広告を作成するためのプロンプト。
...
大胆なタイポグラフィとフラットな黄色の背景で、プレミアムコレクタブルフィギュアを宣伝する高解像度の縦長ソーシャルメディアポスターのためのプロンプト。
...
強いタイポグラフィック階層と可読性を持つブックカバーをデザインするためのプロンプト。
...
提供された論文と規定に基づいて、2026年神経科学会議のカンファレンスポスターを生成するためのプロンプト。
...
グレーのスタジオ空間に浮遊する都市ジオラマを2x2グリッドで生成するプロンプト。
...
### No. 45: 不満なテーマを扱った風刺的システムポスターグリッド
...
不満を感じさせるトピックに関する風刺的エディトリアルポスターを2x2グリッドで生成するプロンプト。
...
都市のランドマークを詳細な手作りペーパージオラマとして描いた、プレミアム縦型旅行ポスターを作成するプロンプト。
...
### No. 47: カルヴィン＆ボブのシネマティックキャラクターシート
...
水彩画風の背景を用いた、分割画面形式のキャラクターシートを作成するプロンプト。
...
バリスタのソフィアというキャラクターの、大胆なタイポグラフィとドラマチックなヒーローイメージを特徴とするプレミアムキャラクターシートを生成するプロンプト。
...
4冊のパブリックドメイン書籍用の、ドラマチックなプロパガンダスタイルの壁紙を2x2グリッドで生成するプロンプト。
...
### No. 50: 4ブランドのコンセプチュアルカットアウェイジオラマ
...
フォーチュン500企業4社のコンセプチュアルカットアウェイジオラマを2x2グリッドで生成するプロンプト。
...
少女と巨大なスニーカーをフィーチャーした、楽しい広告ポスターを生成するためのプロンプト。
...
クリーンなエディトリアルレイアウトで、高級ローズ香水をフィーチャーしたシネマティックな製品ストーリーボードのためのプロンプト。
...
高級ローズ香水をフィーチャーしたシネマティックな製品コマーシャルストーリーボードのためのプロンプト。
...
複雑な機械の3D分解図を示す技術イラストを2x2グリッドで生成するためのプロンプト。
...
本格的な漫画ページの構図を用いて、フルページのカラーでユーモラスな少年漫画イラストを生成するプロンプト。
...
自動エスプレッソマシンの流れを端から端まで追跡する説明用のインフォグラフィック。
...
### No. 57: 正確なキャッチコピー付きストリートウェアキャンペーン広告
...
若者向けストリートウェアキャンペーンの画像で、レイアウト内に正確なキャッチコピーを読みやすく描画する。
...
フラットでベクター風のベーカリーロゴ。クリーンなアルファチャンネル上に中央揃えで配置され、背景はない。
...
縦向きの4コマ漫画。各コマを通じて、始まり、展開、オチが描かれている。
...
### No. 60: iPhoneフレーム内のファーマーズマーケットアプリUI
...
ヘッダー、出品者リスト、特売情報を含む実用的なモバイルアプリのモックアップ。デバイスフレーム内に配置されている。
...
解糖系、クエン酸回路、電子伝達系を結びつけた、高校生物のラベル付き図解。
...
TAM/SAM/SOM図、成長バー、脚注、実際のデータ階層を含むピッチデッキのスライド。
...
ボールドサンセリフで一字一句そのままの見出しを掲載する高速道路ビルボードのモックアップ。
...
縦長4:5アスペクト比で描かれた漫画風の画像。少女が「ねえ、聞いて！」と言い、少年が彼女のセリフフキダシを自分の思考フキダシで覆い隠し、「聞かなかったことにしよう」と思っている場面。
...
少年が「戦わないの？」と言い、鋭い眼差しの少女が彼のセリフフキダシを噛んでいる漫画風のシーン。
...
異なる年代の4つのファッション潮流を、16:9のエディトリアルポスタースタイルで可視化した2x2グリッドを生成するためのプロンプト。
...
4つの異なる政治体制を、パステル調の絵本スタイルで可視化した2x2グリッド画像を生成するためのプロンプト。
...
故人となった4人のチェスのレジェンドに捧げる、英雄的なイラストレーションのトリビュートポスターを2x2グリッドで生成するためのプロンプト。
...
- [Atlas Cloud GPT Image 2.5 Flare text-to-image](https://www.atlascloud.ai/ja/models/openai/gpt-image-2.5-flare/text-to-image?utm_source=github&utm_campaign=awesome-gpt-image-2.5-prompts)
- [Atlas Cloud GPT Image 2.5 Flare edit](https://www.atlascloud.ai/ja/models/openai/gpt-image-2.5-flare/edit?utm_source=github&utm_campaign=awesome-gpt-image-2.5-prompts)
- [Atlas Cloud GPT Image 2.5 Sunburst text-to-image](https://www.atlascloud.ai/ja/models/openai/gpt-image-2.5-sunburst/text-to-image?utm_source=github&utm_campaign=awesome-gpt-image-2.5-prompts)
