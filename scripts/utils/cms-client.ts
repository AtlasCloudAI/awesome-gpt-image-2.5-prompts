...
export interface PromptRecord {
  ...
  category: string;
  video_url?: string;
  // 图像模型:成品图。X 采集侧(fmt_gpt_image2)用 remote_images,策展侧用 image_url,两者都支持
  image_url?: string;
  remote_images?: string[];
  prompt_categories?: string[];
  // 编辑类条目:用于复现「编辑前」那张输入图的底图提示词
  base_prompt?: string;
...
export function sortPrompts(prompts: PromptRecord[]): SortedPromptData {
  ...
  return {
    ...
    stats: {
      ...
      videos: all.filter((prompt) => Boolean(prompt.image_url || (prompt.remote_images ?? []).length)).length,
    },
  };
}
